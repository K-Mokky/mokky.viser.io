...
export function configValidationItems(config) {
    const items = [];
...
function validateAssistant(value, providers, items) {
    if (!section(value, "assistant", items))
        return;
...
function validateStorageLike(path, value, items) {
    if (!section(value, path, items))
        return;
    requireString(value, `${path}.dir`, items);
}
function validateMemory(value, items) {
    if (!section(value, "memory", items))
        return;
...
function validatePersonalization(value, items) {
    if (!section(value, "personalization", items))
        return;
...
function validateSkills(value, items) {
    if (!section(value, "skills", items))
        return;
...
function validatePlugins(value, items) {
    if (!section(value, "plugins", items))
        return;
...
function validateTools(value, items) {
    if (!section(value, "tools", items))
        return;
    ...
    if (!section(shell, "tools.shell", items))
        return;
    ...
    if (!section(webFetch, "tools.webFetch", items))
        return;
    ...
    if (webFetchFirecrawlApiKey?.trim())
        validateWebFetchFirecrawlApiKey(webFetchFirecrawlApiKey, items);
    const webSearch = value.webSearch;
    if (!section(webSearch, "tools.webSearch", items))
        return;
    ...
    if (searxngBaseUrl?.trim())
        validateSearxngBaseUrl(searxngBaseUrl, items);
    ...
    if (braveApiKey?.trim())
        validateBraveApiKey(braveApiKey, items);
    ...
    if (tavilyApiKey?.trim())
        validateTavilyApiKey(tavilyApiKey, items);
    ...
    if (perplexityApiKey?.trim())
        validatePerplexityApiKey(perplexityApiKey, items);
    ...
    if (exaApiKey?.trim())
        validateExaApiKey(exaApiKey, items);
    ...
    if (firecrawlApiKey?.trim())
        validateFirecrawlApiKey(firecrawlApiKey, items);
    if (ollamaBaseUrl?.trim())
        validateOllamaBaseUrl(ollamaBaseUrl, items);
    ...
    if (ollamaApiKey?.trim())
        validateOllamaApiKey(ollamaApiKey, items);
...
function validateBraveApiKey(value, items) {
...
function validateTavilyApiKey(value, items) {
    const trimmed = value.trim();
    if (!trimmed || /[\s\u0000-\u001f\u007f]/u.test(trimmed)) {
        items.push({
            severity: "fail",
...
function validatePerplexityApiKey(value, items) {
    const trimmed = value.trim();
    if (!trimmed || /[\s\u0000-\u001f\u007f]/u.test(trimmed)) {
        items.push({
            severity: "fail",
...
function validateExaApiKey(value, items) {
    const trimmed = value.trim();
    if (!trimmed || /[\s\u0000-\u001f\u007f]/u.test(trimmed)) {
        items.push({
            severity: "fail",
...
function validateFirecrawlApiKey(value, items) {
    const trimmed = value.trim();
    if (!trimmed || /[\s\u0000-\u001f\u007f]/u.test(trimmed)) {
        items.push({
            severity: "fail",
...
function validateOllamaApiKey(value, items) {
    const trimmed = value.trim();
    if (!trimmed || /[\s\u0000-\u001f\u007f]/u.test(trimmed)) {
        items.push({
            severity: "fail",
...
function validateWebFetchFirecrawlApiKey(value, items) {
    const trimmed = value.trim();
    if (!trimmed || /[\s\u0000-\u001f\u007f]/u.test(trimmed)) {
        items.push({
            severity: "fail",
...
function validateSearxngBaseUrl(value, items) {
    let url;
    try {
...
function validateOllamaBaseUrl(value, items) {
    let url;
    try {
        url = new URL(value.trim());
    }
...
function validateBrowserUseBaseUrl(value, items) {
    let url;
    try {
        url = new URL(value.trim());
    }
...
function validateHttpsApiBaseUrl(value, path, tlsNext, items) {
    let url;
    try {
        url = new URL(value.trim());
    }
    ...
    if (url.protocol !== "https:")
        itemsPushBaseUrlError(items, path, "must use https", tlsNext);
    if (url.username || url.password)
        itemsPushBaseUrlError(items, path, "must not include URL credentials");
    if (url.search || url.hash)
        itemsPushBaseUrlError(items, path, "must not include query or hash");
    if (url.pathname && url.pathname !== "/")
        itemsPushBaseUrlError(items, path, "must not include a path");
}
function itemsPushBaseUrlError(items, path, message, next) {
    items.push({ severity: "fail", path, message, ...(next ? { next } : {}) });
}
function validateLocalCdpBaseUrl(value, items) {
    let url;
    try {
        url = new URL(value.trim());
    }
...
function validateBrowserTaskAllowedDomain(value, path, items) {
...
function validateTickSection(path, value, items) {
    if (!section(value, path, items))
        return;
...
function validateJobs(value, items) {
    if (!section(value, "jobs", items))
        return;
...
function validateWebDashboard(value, items) {
    if (!section(value, "webDashboard", items))
        return;
    ...
    if (value.authToken !== undefined)
        requireString(value, "webDashboard.authToken", items);
...
function validateAccess(value, items) {
    if (!section(value, "access", items))
        return;
...
function validateActions(value, items) {
    if (!section(value, "actions", items))
        return;
...
function validateBrowserTaskAction(value, items) {
    if (!section(value, "actions.browserTask", items))
        return;
    ...
    if (baseUrl)
        validateBrowserUseBaseUrl(baseUrl, items);
    if (browserbaseBaseUrl)
        validateHttpsApiBaseUrl(browserbaseBaseUrl, "actions.browserTask.browserbaseBaseUrl", "Browserbase session API credentials must only be sent to a TLS endpoint.", items);
    if (firecrawlBaseUrl)
        validateHttpsApiBaseUrl(firecrawlBaseUrl, "actions.browserTask.firecrawlBaseUrl", "Firecrawl browser-task credentials must only be sent to a TLS endpoint.", items);
    if (localCdpBaseUrl)
        validateLocalCdpBaseUrl(localCdpBaseUrl, items);
...
function validateTelegram(value, items) {
    if (!section(value, "connectors.telegram", items))
        return;
...
function validateDiscord(value, items) {
    if (!section(value, "connectors.discord", items))
        return;
...
function validateSlack(value, items) {
    if (!section(value, "connectors.slack", items))
        return;
...
function validateMatrix(value, items) {
    if (!section(value, "connectors.matrix", items))
        return;
...
function validateSignal(value, items) {
    if (!section(value, "connectors.signal", items))
        return;
...
function validateImessage(value, items) {
    if (!section(value, "connectors.imessage", items))
        return;
...
function validateWhatsapp(value, items) {
    if (!section(value, "connectors.whatsapp", items))
        return;
...
function validateLine(value, items) {
    if (!section(value, "connectors.line", items))
        return;
...
function validateKakaotalk(value, items) {
    if (!section(value, "connectors.kakaotalk", items))
        return;
...
function validateWebhookSender(value, path, label, items) {
    if (!section(value, path, items))
        return;
    requireBoolean(value, `${path}.enabled`, items);
...
function validateWebex(value, items) {
    if (!section(value, "connectors.webex", items))
        return;
...
function validateZalo(value, items) {
    if (!section(value, "connectors.zalo", items))
        return;
...
function validateIrc(value, items) {
    if (!section(value, "connectors.irc", items))
        return;
...
function validateTwitch(value, items) {
    if (!section(value, "connectors.twitch", items))
        return;
...
function validateNtfy(value, items) {
    if (!section(value, "connectors.ntfy", items))
        return;
...
function validateMastodon(value, items) {
    if (!section(value, "connectors.mastodon", items))
        return;
...
function validateNextcloudTalk(value, items) {
    if (!section(value, "connectors.nextcloudTalk", items))
        return;
...
function validateZulip(value, items) {
    if (!section(value, "connectors.zulip", items))
        return;
...
function validateEmail(value, items) {
    if (!section(value, "connectors.email", items))
        return;
...
function validateGitHub(value, items) {
    if (!section(value, "connectors.github", items))
        return;
...
function validateTodoist(value, items) {
    if (!section(value, "connectors.todoist", items))
        return;
...
function validateNotion(value, items) {
    if (!section(value, "connectors.notion", items))
        return;
...
function validateObsidian(value, items) {
    if (!section(value, "connectors.obsidian", items))
        return;
...
function validateHomeAssistant(value, items) {
    if (!section(value, "connectors.homeAssistant", items))
        return;
...
function validateBoundedConnectorMs(value, path, maxMs, next, items) {
...
function validateConnectorRateLimit(value, path, items) {
...
function validateConnectorInputLimit(value, path, items) {
...
function validateProviders(value, items) {
    ...
    for (const [providerId, provider] of entries) {
        ...
        if (!section(provider, path, items))
            continue;
...
function section(value, path, items) {
    if (isPlainObject(value))
        return true;
    items.push({ severity: "fail", path, message: "must be an object" });
    return false;
}
function requireString(value, path, items) {
...
function optionalString(value, path, items) {
    const raw = getLeaf(value, path);
    if (raw === undefined)
        return undefined;
...
function requireBoolean(value, path, items) {
    const raw = getLeaf(value, path);
...
function requirePositiveInteger(value, path, items) {
    const raw = getLeaf(value, path);
...
function optionalPositiveInteger(value, path, items) {
    const raw = getLeaf(value, path);
    if (raw === undefined)
        return undefined;
    if (Number.isInteger(raw) && raw > 0)
...
function requireStringArray(value, path, items) {
    const raw = getLeaf(value, path);
...
function validateOptionalStringRecord(value, path, items) {
    if (value === undefined)
        return;
    validateStringRecord(value, path, items, "must be an object of string values when present");
}
function validateStringRecord(value, path, items, objectMessage = "must be an object of string values") {
    if (!isPlainObject(value)) {
