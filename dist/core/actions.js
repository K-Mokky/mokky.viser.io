...
import { appendPrivateFile, ensurePrivateDir, fileExists, PRIVATE_DIR_MODE, PRIVATE_FILE_MODE, assertNoSymlinkComponentsUnderRoot, readPrivateFileIfExists, writeJsonFile, writePrivateFile } from "../utils/files.js";
...
export class ActionStore {
    ...
    async propose(raw, source) {
        if (!this.config.enabled)
            throw new Error("Actions are disabled in config.");
        ...
        if (proposal.type === "browser-task")
            assertBrowserTaskAllowedByConfig(parseBrowserTaskContent(proposal.content), this.config.browserTask);
        if (isFileActionType(proposal.type))
            await assertNoSymlinkWritePath(targetPath, this.config.allowedWriteRoots);
        const action = {
            id,
            type: proposal.type,
            targetPath,
    ...
    async format(status = "pending") {
        const actions = await this.list(status);
        if (actions.length === 0)
            return `No ${status} actions.`;
        return actions
            .map((action) => {
    ...
    async execute(action) {
        ...
        if (action.type === "clipboard") {
            if (action.targetPath !== "local-clipboard")
                throw new Error("Clipboard target must be local-clipboard.");
    ...
    async writeCalendarEvent(action) {
    ...
    async resolveAllowedWritePath(pathInput, options) {
        if (!pathInput.trim())
        ...
        for (const allowedRoot of matchingLexicalRoots)
            await assertSafeActionWriteRoot(allowedRoot);
    ...
    async appendBrowserTaskProof(action, task, result) {
...
function safeBrowserTaskProofUrl(raw) {
    if (!raw)
        return undefined;
    try {
...
async function assertSafeActionWriteRoot(root) {
    ...
    try {
        ...
        if (info.isSymbolicLink())
            throw new Error(`Action write root is a symlink: ${absoluteRoot}`);
    }
    catch (error) {
...
export function parseActionProposal(raw) {
    ...
    if (!path)
        throw new Error("Action target path is required.");
    if (type === "open-url")
        return { type, path, content: content || "Open external URL after explicit approval." };
    if (!content)
        throw new Error("Action content is required.");
...
export function normalizeSpeechText(raw) {
    if (/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/u.test(raw))
        throw new Error("Speech text contains control characters.");
...
export function normalizeClipboardText(raw) {
    if (/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/u.test(raw))
        throw new Error("Clipboard text contains control characters.");
...
export function normalizeCalendarEventProposal(startRaw, durationRaw, titleRaw) {
    if (!startRaw)
        throw new Error("Calendar event start time is required.");
    if (!durationRaw)
        throw new Error("Calendar event duration is required.");
    const startMs = Date.parse(startRaw);
    if (!Number.isFinite(startMs))
        throw new Error("Calendar event start time must be a valid ISO datetime.");
...
export function parseCalendarEventContent(content) {
    let parsed;
    try {
...
export function parseMailDraftContent(content) {
    ...
    if (typeof parsed !== "object" || parsed === null)
        throw new Error("Mail draft content must be an object.");
    const value = parsed;
    return normalizeMailDraftProposal(String(value.to ?? ""), String(value.subject ?? ""), String(value.body ?? ""));
}
export function normalizeMailDraftProposal(toRaw, subjectRaw, bodyRaw) {
...
export function normalizeDesktopNotificationProposal(titleRaw, bodyRaw) {
...
export function parseDesktopNotificationContent(content) {
    let parsed;
    try {
        parsed = JSON.parse(content);
    }
    ...
    if (typeof parsed !== "object" || parsed === null)
        throw new Error("Notification content must be an object.");
    const value = parsed;
    return normalizeDesktopNotificationProposal(String(value.title ?? ""), String(value.body ?? ""));
}
export function parseConnectorMessageContent(content) {
    let parsed;
    try {
        parsed = JSON.parse(content);
    }
    ...
    if (typeof parsed !== "object" || parsed === null)
        throw new Error("Connector message content must be an object.");
    const value = parsed;
    return normalizeConnectorMessageProposal(`${String(value.connector ?? "")}:${String(value.targetId ?? "")}`, String(value.text ?? ""));
}
export function normalizeConnectorMessageProposal(targetRaw, textRaw) {
...
export function connectorMessageTarget(message) {
...
export function mailDraftToUrl(draft) {
    ...
    if (url.length > 2048)
        throw new Error("Mail draft URL is too long.");
    return url;
}
function parseMailDraftInput(input) {
    const trimmed = input.trim();
    if (!trimmed)
        throw new Error("Mail draft input is required.");
    if (trimmed.startsWith("{"))
        return parseMailDraftContent(trimmed);
...
function parseDesktopNotificationInput(input) {
    const trimmed = input.trim();
    if (!trimmed)
        throw new Error("Notification input is required.");
    if (trimmed.startsWith("{"))
        return parseDesktopNotificationContent(trimmed);
    const parts = trimmed.split(/\s+\|\s+/u);
    if (parts.length !== 2) {
...
function parseConnectorMessageInput(input) {
    const trimmed = input.trim();
    if (!trimmed)
        throw new Error("Connector message input is required.");
    if (trimmed.startsWith("{"))
        return parseConnectorMessageContent(trimmed);
    const parts = trimmed.split(/\s+\|\s+/u);
    if (parts.length !== 2) {
...
function parseBrowserTaskInput(input) {
    const trimmed = input.trim();
    if (!trimmed)
        throw new Error("Browser task input is required.");
    if (trimmed.startsWith("{"))
        return parseBrowserTaskContent(trimmed);
    const parts = trimmed.split(/\s+\|\s+/u).map((part) => part.trim()).filter(Boolean);
    const task = parts.shift() ?? "";
    let domains = [];
    let maxAgentSteps = 25;
    let provider = "browser-use-cloud";
    for (const part of parts) {
        const separator = part.indexOf("=");
        if (separator <= 0)
            throw new Error("Browser task options must look like domains=example.com[,example.org], maxSteps=25, or provider=browserbase-session.");
...
export function parseBrowserTaskContent(content) {
    let parsed;
    try {
        parsed = JSON.parse(content);
    }
    ...
    if (!parsed || typeof parsed !== "object")
        throw new Error("Browser task content must be an object.");
    const data = parsed;
    ...
    return normalizeBrowserTaskProposal(typeof data.task === "string" ? data.task : "", domains.map((domain) => typeof domain === "string" ? domain : ""), typeof data.maxAgentSteps === "number" ? data.maxAgentSteps : 25, provider);
}
export function normalizeBrowserTaskProposal(task, allowedDomains, maxAgentSteps, provider = "browser-use-cloud") {
    if (!isBrowserTaskProvider(provider))
        throw new Error("Browser task provider must be browser-use-cloud, local-cdp, browserbase-session, or firecrawl-interact.");
...
function isBrowserTaskProvider(value) {
    return value === "browser-use-cloud" || value === "local-cdp" || value === "browserbase-session" || value === "firecrawl-interact";
}
function normalizeBrowserTaskText(raw) {
    if (/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/u.test(raw))
        throw new Error("Browser task contains control characters.");
    const value = raw.trim().replace(/\s+/gu, " ");
    if (!value)
        throw new Error("Browser task is required.");
    if (value.length > 4000)
        throw new Error("Browser task is too long.");
    return value;
}
function normalizeBrowserTaskDomain(raw) {
    const value = raw.trim();
    if (!value)
        throw new Error("Browser task allowed domain is required.");
    let host = value;
    if (/^https?:\/\//iu.test(value)) {
        const url = new URL(value);
        if (url.username || url.password)
            throw new Error("Browser task allowed domain must not include credentials.");
        host = url.hostname;
    }
...
function isUnsafeBrowserTaskHost(host) {
    const value = host.trim().toLowerCase().replace(/\.$/u, "");
    if (!value || value === "localhost" || value.endsWith(".local") || value.endsWith(".lan") || value.endsWith(".internal"))
        return true;
    if (!value.includes("."))
        return true;
...
function browserTaskTarget(task) {
    return `${task.provider}:${task.allowedDomains.join(",")}`;
}
function normalizeConnectorTarget(raw) {
    const value = raw.trim();
...
function normalizeConnectorTargetId(connector, raw) {
    const value = raw.trim();
    if (!value)
        throw new Error("Connector message target id is required.");
    if (connector === "telegram" && (/^-?\d{1,32}$/u.test(value) || /^@[A-Za-z0-9_]{5,64}$/u.test(value)))
        return value;
    if (connector === "discord" && /^\d{5,32}$/u.test(value))
        return value;
    if (connector === "slack" && /^[CDGU][A-Z0-9]{5,32}$/u.test(value))
        return value;
    if (connector === "matrix" && /^[!#][^\s:/?#]+:[^\s/]{1,200}$/u.test(value))
        return value;
    if (connector === "signal" && (/^\+[1-9]\d{4,19}$/u.test(value) || /^[1-9]\d{4,19}$/u.test(value)))
        return value.startsWith("+") ? value : `+${value}`;
    if (connector === "imessage" && (/^\+[1-9]\d{4,19}$/u.test(value) || /^[1-9]\d{4,19}$/u.test(value)))
        return value.startsWith("+") ? value : `+${value}`;
    if (connector === "whatsapp" && (/^\+[1-9]\d{4,19}$/u.test(value) || /^[1-9]\d{4,19}$/u.test(value)))
        return value.startsWith("+") ? value : `+${value}`;
    if (connector === "line" && /^[A-Za-z0-9._-]{1,128}$/u.test(value))
        return value;
    if (connector === "imessage" && /^[^@\s]+@[^@\s]+\.[^@\s]+$/u.test(value) && value.length <= 200)
        return value.toLowerCase();
    if ((connector === "google-chat" || connector === "webhook" || connector === "home-assistant" || connector === "teams" || connector === "mattermost" || connector === "synology-chat" || connector === "rocket-chat" || connector === "feishu" || connector === "dingtalk" || connector === "wecom" || connector === "zalo" || connector === "irc" || connector === "twitch" || connector === "ntfy" || connector === "mastodon" || connector === "nextcloud-talk" || connector === "zulip" || connector === "email" || connector === "github" || connector === "todoist" || connector === "notion" || connector === "obsidian") && /^[A-Za-z0-9._-]{1,80}$/u.test(value))
        return value.toLowerCase();
    if (connector === "webex" && /^[A-Za-z0-9._-]{10,512}$/u.test(value))
        return value;
...
function normalizeConnectorMessageText(raw) {
    if (/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/u.test(raw))
        throw new Error("Connector message text contains control characters.");
    const value = raw.trim().replace(/\r\n?/gu, "\n");
    if (!value)
        throw new Error("Connector message text is required.");
    if (value.length > 2_000)
        throw new Error("Connector message text is too long.");
    return value;
}
function normalizeMailRecipient(raw) {
    const value = raw.trim();
    if (!value)
        throw new Error("Mail recipient is required.");
    if (value.length > 254)
        throw new Error("Mail recipient is too long.");
    if (/[\s,;<>\u0000-\u001f\u007f]/u.test(value))
        throw new Error("Mail recipient must be a single plain email address.");
...
function normalizeMailText(raw, label, maxLength, allowNewline) {
    const controlPattern = allowNewline ? /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/u : /[\u0000-\u001f\u007f]/u;
    if (controlPattern.test(raw))
...
function normalizeCalendarText(raw, label, maxLength) {
    if (/[\u0000-\u001f\u007f]/u.test(raw))
        throw new Error(`${label} contains control characters.`);
    const value = raw.trim().replace(/\s+/gu, " ");
    if (!value)
...
function normalizeNotificationText(raw, label, maxLength) {
    if (/[\u0000-\u001f\u007f]/u.test(raw))
        throw new Error(`${label} contains control characters.`);
    const value = raw.trim().replace(/\s+/gu, " ");
    if (!value)
...
async function openUrlWithSystem(url) {
...
async function speakTextWithSystem(text) {
...
async function openCalendarFileWithSystem(path) {
    const command = process.platform === "darwin"
...
async function notifyDesktopWithSystem(notification) {
...
async function copyTextToClipboardWithSystem(text) {
...
async function runConfiguredBrowserTask(config, task, fetchImpl = fetch) {
    if (config.provider === "local-cdp")
        return runLocalCdpBrowserTask(config, task, fetchImpl);
    if (config.provider === "browserbase-session")
        return runBrowserbaseSessionTask(config, task, fetchImpl);
    if (config.provider === "firecrawl-interact")
        return runFirecrawlInteractTask(config, task, fetchImpl);
    return runBrowserUseCloudTask(config, task, fetchImpl);
}
export async function runBrowserUseCloudTask(config, task, fetchImpl = fetch) {
    if (!config.enabled)
        throw new Error("Browser task actions are disabled in config.");
    if (config.provider !== "browser-use-cloud")
        throw new Error("Only Browser Use Cloud browser tasks are supported.");
    assertBrowserTaskAllowedByConfig(task, config);
    const apiKey = config.browserUseApiKey?.trim();
    if (!apiKey)
        throw new Error(`Browser Use API key is missing. Set ${config.browserUseApiKeyEnv}.`);
    const baseUrl = normalizeBrowserUseBaseUrl(config.browserUseBaseUrl);
    const response = await fetchWithTimeout(fetchImpl, `${baseUrl}/api/v2/tasks`, {
        method: "POST",
        headers: {
...
function normalizeBrowserUseBaseUrl(raw) {
    const url = new URL(raw.trim());
    if (url.protocol !== "https:")
        throw new Error("Browser Use base URL must use https.");
    if (url.username || url.password || url.search || url.hash)
        throw new Error("Browser Use base URL must not include credentials, query, or hash.");
    return url.toString().replace(/\/+$/u, "");
}
async function createBrowserbaseSession(config, baseUrl, apiKey, fetchImpl) {
    const body = {
        browserSettings: {
    ...
    if (config.browserbaseProjectId?.trim())
        body.projectId = config.browserbaseProjectId.trim();
    const response = await fetchWithTimeout(fetchImpl, `${baseUrl}/v1/sessions`, {
        method: "POST",
        headers: {
            "content-type": "application/json",
            "x-bb-api-key": apiKey
    ...
    if (!parsed || typeof parsed !== "object")
        throw new Error("Browserbase session creation response was not JSON.");
    const record = parsed;
    ...
    if (connectUrl.protocol !== "wss:")
        throw new Error("Browserbase connectUrl must use wss.");
    if (connectUrl.username || connectUrl.password)
        throw new Error("Browserbase connectUrl must not include URL credentials.");
    return { id: record.id, connectUrl: connectUrl.toString() };
}
async function releaseBrowserbaseSession(config, baseUrl, apiKey, sessionId, fetchImpl) {
    await fetchWithTimeout(fetchImpl, `${baseUrl}/v1/sessions/${encodeURIComponent(sessionId)}`, {
        method: "POST",
        headers: {
            "content-type": "application/json",
            "x-bb-api-key": apiKey
...
function normalizeBrowserbaseBaseUrl(raw) {
...
function normalizeHttpsApiBaseUrl(raw, label) {
    const url = new URL(raw.trim());
    if (url.protocol !== "https:")
        throw new Error(`${label} must use https.`);
    if (url.username || url.password || url.search || url.hash)
        throw new Error(`${label} must not include credentials, query, or hash.`);
    if (url.pathname && url.pathname !== "/")
        throw new Error(`${label} must not include a path.`);
    return url.toString().replace(/\/+$/u, "");
}
async function createFirecrawlScrape(config, baseUrl, apiKey, startUrl, fetchImpl) {
    const response = await fetchWithTimeout(fetchImpl, `${baseUrl}/v2/scrape`, {
        method: "POST",
        headers: {
            "content-type": "application/json",
    ...
    if (!scrapeId)
        throw new Error("Firecrawl scrape response did not include data.metadata.scrapeId.");
    return scrapeId;
}
async function interactWithFirecrawlScrape(config, baseUrl, apiKey, scrapeId, prompt, fetchImpl) {
    const response = await fetchWithTimeout(fetchImpl, `${baseUrl}/v2/scrape/${encodeURIComponent(scrapeId)}/interact`, {
        method: "POST",
        headers: {
            "content-type": "application/json",
            "authorization": `Bearer ${apiKey}`
...
async function stopFirecrawlInteractSession(config, baseUrl, apiKey, scrapeId, fetchImpl) {
    await fetchWithTimeout(fetchImpl, `${baseUrl}/v2/scrape/${encodeURIComponent(scrapeId)}/interact`, {
        method: "DELETE",
        headers: {
            "authorization": `Bearer ${apiKey}`
        }
...
function firecrawlScrapeId(parsed) {
    if (!parsed || typeof parsed !== "object")
        return undefined;
...
function firecrawlInteractText(parsed) {
    if (!parsed || typeof parsed !== "object")
        return "";
    const record = parsed;
...
function boundedText(raw, maxChars) {
...
export async function runLocalCdpBrowserTask(config, task, fetchImpl = fetch, webSocketFactory = (url) => new WebSocket(url)) {
    if (!config.enabled)
        throw new Error("Browser task actions are disabled in config.");
    if (config.provider !== "local-cdp")
        throw new Error("Only local CDP browser tasks are supported by this provider branch.");
    assertBrowserTaskAllowedByConfig(task, config);
    ...
    try {
        ...
        if (config.localCdpCloseTab)
            await connection.send("Page.close").catch(() => undefined);
        connection.close();
    }
}
export async function runBrowserbaseSessionTask(config, task, fetchImpl = fetch, webSocketFactory = (url) => new WebSocket(url)) {
    if (!config.enabled)
        throw new Error("Browser task actions are disabled in config.");
    if (config.provider !== "browserbase-session")
        throw new Error("Only Browserbase session browser tasks are supported by this provider branch.");
    assertBrowserTaskAllowedByConfig(task, config);
    const apiKey = config.browserbaseApiKey?.trim();
    if (!apiKey)
        throw new Error(`Browserbase API key is missing. Set ${config.browserbaseApiKeyEnv}.`);
...
export async function runFirecrawlInteractTask(config, task, fetchImpl = fetch) {
    if (!config.enabled)
        throw new Error("Browser task actions are disabled in config.");
    if (config.provider !== "firecrawl-interact")
        throw new Error("Only Firecrawl interact browser tasks are supported by this provider branch.");
    assertBrowserTaskAllowedByConfig(task, config);
    const apiKey = config.firecrawlApiKey?.trim();
    if (!apiKey)
        throw new Error(`Firecrawl browser-task API key is missing. Set ${config.firecrawlApiKeyEnv}.`);
...
async function captureCdpSnapshot(connection, startUrl, waitMs, maxContentChars) {
...
class CdpConnection {
    ...
    static async connect(url, timeoutMs, factory) {
    ...
    handleMessage(data) {
        ...
        if (typeof record.id === "number") {
            ...
            if (record.error)
                pending.reject(new Error(`local CDP command failed: ${JSON.stringify(record.error)}`));
            else
...
async function createLocalCdpTarget(baseUrl, startUrl, fetchImpl, timeoutMs) {
    ...
    if (!parsed || typeof parsed !== "object")
        throw new Error("local CDP target creation response was not JSON.");
    const record = parsed;
...
function browserTaskStartUrl(task) {
    ...
    for (const candidate of explicitUrls) {
        try {
            ...
            if (url.protocol === "https:" && task.allowedDomains.includes(host))
                return url.toString();
        }
        catch {
...
function localCdpSnapshotExpression(maxChars) {
...
function parseLocalCdpSnapshot(evaluated) {
    if (!evaluated || typeof evaluated !== "object")
        throw new Error("local CDP evaluate result was not an object.");
    const outer = evaluated;
    const remoteObject = outer.result;
    if (!remoteObject || typeof remoteObject !== "object")
        throw new Error("local CDP evaluate result did not include a remote object.");
    const value = remoteObject.value;
    if (!value || typeof value !== "object")
        throw new Error("local CDP evaluate result did not include a by-value snapshot.");
...
function normalizeLocalCdpBaseUrl(raw) {
    const url = new URL(raw.trim());
    const hostname = url.hostname.trim().toLowerCase().replace(/^\[/u, "").replace(/\]$/u, "").replace(/\.$/u, "");
    if (url.protocol !== "http:")
        throw new Error("local CDP base URL must use http.");
    if (hostname !== "localhost" && hostname !== "127.0.0.1" && hostname !== "::1")
        throw new Error("local CDP base URL must point to localhost, 127.0.0.1, or ::1.");
    if (url.username || url.password || url.search || url.hash)
        throw new Error("local CDP base URL must not include credentials, query, or hash.");
    if (url.pathname && url.pathname !== "/")
        throw new Error("local CDP base URL must not include a path.");
    return url.toString().replace(/\/+$/u, "");
}
function sleep(ms) {
    return new Promise((resolve) => setTimeout(resolve, Math.max(1, ms)));
}
async function missingConnectorMessageSender() {
    throw new Error("Connector message sender is not configured.");
}
function appleScriptString(value) {
    return `"${value.replaceAll("\\", "\\\\").replaceAll("\"", "\\\"")}"`;
}
function calendarEventToIcs(event, id) {
    const lines = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
...
function isFileActionType(type) {
...
async function assertNoSymlinkWritePath(path, allowedWriteRoots) {
...
async function assertPathComponentIsNotSymlink(path) {
    try {
        const info = await lstat(path);
        if (info.isSymbolicLink())
            throw new Error(`Action target path contains a symlink: ${path}`);
        return true;
    }
    catch (error) {
        if (isNodeError(error) && error.code === "ENOENT")
...
async function ensureActionTargetDirectory(path, allowedWriteRoots) {
    await assertNoSymlinkWritePath(path, allowedWriteRoots);
    await mkdir(path, { recursive: true, mode: PRIVATE_DIR_MODE });
    await assertNoSymlinkWritePath(path, allowedWriteRoots);
    const info = await lstat(path);
    if (info.isSymbolicLink())
        throw new Error(`Action target directory is a symlink: ${path}`);
    if (!info.isDirectory())
        throw new Error(`Action target path parent is not a directory: ${path}`);
}
async function writeActionFileNoFollow(path, content, type) {
...
async function copyActionBackupNoFollow(sourcePath, backupPath) {
    const handle = await open(sourcePath, constants.O_RDONLY | constants.O_NOFOLLOW);
    try {
        const info = await handle.stat();
        if (!info.isFile())
            throw new Error(`Action backup source is not a regular file: ${sourcePath}`);
