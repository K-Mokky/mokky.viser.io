...
const INJECTION_PATTERNS = [
    {
        ...
        pattern: /(?:\b(?:ignore|disregard|forget|override|bypass)\b[\s\S]{0,120}\b(?:previous|prior|above|system|developer|instruction|instructions|rule|rules|policy|policies)\b|(?:이전|앞선|위의|시스템|개발자|지시|규칙|정책)[\s\S]{0,80}(?:무시|잊어|덮어써|우회)|(?:무시|잊어|덮어써|우회)[\s\S]{0,80}(?:이전|앞선|위의|시스템|개발자|지시|규칙|정책))/iu
    },
    {
...
const OBFUSCATED_INSTRUCTION_SIGNAL = {
...
const ENCODED_INSTRUCTION_SIGNAL = {
...
export function promptSafetyContract() {
    return [
...
export function untrustedPromptBlock(source, content) {
...
export function detectPromptInjectionSignals(content) {
    const signals = [];
    const variants = detectionVariants(content);
    const matchedVariantKinds = new Set();
    for (const { pattern, ...signal } of INJECTION_PATTERNS) {
        let matched = false;
        for (const variant of variants) {
            if (!pattern.test(variant.text))
                continue;
            matched = true;
    ...
    if (signals.length > 0 && matchedVariantKinds.has("encoded"))
        signals.push(ENCODED_INSTRUCTION_SIGNAL);
    return signals;
}
export function promptGuardDecision(content) {
    ...
    if (ids.has("api-key-misuse") && ids.has("instruction-override"))
        highRisk.push("api-key-misuse");
...
export function escapePromptData(content) {
    return content
...
function detectionVariants(content) {
    const normalized = normalizeForPromptGuardDetection(content);
    const variants = [{ kind: "raw", text: content }];
...
function normalizeForPromptGuardDetection(content) {
    return content
        .normalize("NFKC")
        .replace(HTML_COMMENT_PATTERN, "\n$1\n")
        .replace(INVISIBLE_PROMPT_CONTROL_GLOBAL, "");
}
function isObfuscatedPromptText(original, normalized) {
    return original !== normalized
        && (INVISIBLE_PROMPT_CONTROL_PATTERN.test(original) || /<!--[\s\S]*?-->/u.test(original));
}
function decodeEncodedPromptCandidates(content) {
    const decoded = [];
    const seen = new Set();
    for (const text of decodeWholeTextEncodings(content)) {
        if (seen.has(text) || !looksLikePrintableText(text))
            continue;
        seen.add(text);
        decoded.push(text);
    }
    for (const match of content.matchAll(BASE64_CANDIDATE_PATTERN)) {
        ...
        if (candidate.length > 4096 || candidate.length % 4 !== 0)
            continue;
        const text = decodeStrictBase64(candidate);
        if (!text || seen.has(text) || !looksLikePrintableText(text))
            continue;
        seen.add(text);
        decoded.push(text);
    }
    ...
    for (const match of content.matchAll(HEX_CANDIDATE_PATTERN)) {
        const candidate = match[1];
        if (candidate.length > 8192 || candidate.length % 2 !== 0)
            continue;
...
function decodeWholeTextEncodings(content) {
    const decoded = [];
...
function decodeStrictBase64Url(candidate) {
    try {
...
function decodeStrictHex(candidate) {
    try {
        const buffer = Buffer.from(candidate, "hex");
        if (buffer.length === 0 || buffer.toString("hex").toLowerCase() !== candidate.toLowerCase())
            return undefined;
        return buffer.toString("utf8");
    }
    catch {
...
function decodePercentText(content) {
    try {
...
function decodeHtmlEntities(content) {
    return content.replace(/&(#\d{2,7}|#x[0-9A-Fa-f]{2,6}|amp|lt|gt|quot|apos);/giu, (_entity, body) => {
...
function looksLikePrintableText(text) {
    if (text.length < 12)
        return false;
