...
export function providerSmokeCommand(provider, prompt = "Reply with exactly: VISER_OK") {
...
export function providerIssueAdvice(provider, detail) {
...
async function providerGuideBlock(provider, options) {
...
function selectProviders(config, providerId) {
    if (providerId)
        return config.providers[providerId] ? [config.providers[providerId]] : [];
    return Object.values(config.providers);
}
function formatProbe(provider, installed, requested, probe) {
    if (!requested)
        return `skipped (run \`viser provider-guide ${provider.id} --probe\`)`;
    if (!installed)
        return "skipped because command is missing";
    if (!probe)
        return "not run";
    return `${probe.ok ? "pass" : "fail"} in ${probe.elapsedMs}ms — ${probe.detail}`;
}
function providerTroubleshooting(provider, installed, probe) {
    const notes = [];
    if (!installed) {
    ...
    if (probe && !probe.ok)
        notes.push(...providerIssueAdvice(provider, probe.detail));
    return notes;
}
