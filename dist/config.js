...
import { assertNoSymlinkComponentsUnderRoot, readJsonFile } from "./utils/files.js";
import { assertValidConfig } from "./config-validation.js";
import { parseDingTalkWebhookUrlMap } from "./connectors/dingtalk.js";
import { parseFeishuWebhookUrlMap } from "./connectors/feishu.js";
import { parseEmailRecipientMap } from "./connectors/email.js";
import { parseGenericWebhookUrlMap } from "./connectors/generic-webhook.js";
import { parseGitHubIssueTargetMap } from "./connectors/github.js";
import { parseTodoistProjectMap } from "./connectors/todoist.js";
import { parseNotionPageMap } from "./connectors/notion.js";
import { parseObsidianNoteMap } from "./connectors/obsidian.js";
import { parseHomeAssistantServiceMap } from "./connectors/home-assistant.js";
import { parseIrcChannelMap } from "./connectors/irc.js";
import { parseWebhookUrlMap } from "./connectors/google-chat.js";
import { parseMattermostWebhookUrlMap } from "./connectors/mattermost.js";
import { parseNextcloudTalkRoomMap } from "./connectors/nextcloud-talk.js";
import { parseNtfyTopicMap } from "./connectors/ntfy.js";
import { normalizeMastodonVisibility, parseMastodonTargetMap } from "./connectors/mastodon.js";
import { parseRocketChatWebhookUrlMap } from "./connectors/rocket-chat.js";
import { parseSynologyChatWebhookUrlMap } from "./connectors/synology-chat.js";
import { parseTeamsWebhookUrlMap } from "./connectors/teams.js";
import { parseTwitchChannelMap } from "./connectors/twitch.js";
import { parseWeComWebhookUrlMap } from "./connectors/wecom.js";
import { parseZaloRecipientMap } from "./connectors/zalo.js";
import { parseZulipTargetMap } from "./connectors/zulip.js";
...
export async function loadConfig(options = {}) {
    ...
    if (env.VISER_PROVIDER)
        merged.assistant.defaultProvider = env.VISER_PROVIDER;
    assertValidConfig(merged);
    ...
    if (browserUseApiKeyEnvName && env[browserUseApiKeyEnvName])
        merged.actions.browserTask.browserUseApiKey = env[browserUseApiKeyEnvName];
    if (browserbaseApiKeyEnvName && env[browserbaseApiKeyEnvName])
        merged.actions.browserTask.browserbaseApiKey = env[browserbaseApiKeyEnvName];
    if (browserbaseProjectIdEnvName && env[browserbaseProjectIdEnvName])
        merged.actions.browserTask.browserbaseProjectId = env[browserbaseProjectIdEnvName];
    if (browserTaskFirecrawlApiKeyEnvName && env[browserTaskFirecrawlApiKeyEnvName])
        merged.actions.browserTask.firecrawlApiKey = env[browserTaskFirecrawlApiKeyEnvName];
    if (browserCdpBaseUrlEnvName && env[browserCdpBaseUrlEnvName])
        merged.actions.browserTask.localCdpBaseUrl = env[browserCdpBaseUrlEnvName];
    if (telegramEnvName && env[telegramEnvName])
        merged.connectors.telegram.botToken = env[telegramEnvName];
    if (discordEnvName && env[discordEnvName])
        merged.connectors.discord.botToken = env[discordEnvName];
    if (slackBotEnvName && env[slackBotEnvName])
        merged.connectors.slack.botToken = env[slackBotEnvName];
    if (slackAppEnvName && env[slackAppEnvName])
        merged.connectors.slack.appToken = env[slackAppEnvName];
    if (slackBotUserIdEnvName && env[slackBotUserIdEnvName])
        merged.connectors.slack.botUserId = env[slackBotUserIdEnvName];
    if (matrixHomeserverEnvName && env[matrixHomeserverEnvName])
        merged.connectors.matrix.homeserverUrl = env[matrixHomeserverEnvName];
    if (matrixAccessTokenEnvName && env[matrixAccessTokenEnvName])
        merged.connectors.matrix.accessToken = env[matrixAccessTokenEnvName];
    if (matrixUserIdEnvName && env[matrixUserIdEnvName])
        merged.connectors.matrix.userId = env[matrixUserIdEnvName];
    if (signalCommandEnvName && env[signalCommandEnvName])
        merged.connectors.signal.command = env[signalCommandEnvName];
    if (signalAccountEnvName && env[signalAccountEnvName])
        merged.connectors.signal.account = env[signalAccountEnvName];
    if (imessageSqliteCommandEnvName && env[imessageSqliteCommandEnvName])
        merged.connectors.imessage.sqliteCommand = env[imessageSqliteCommandEnvName];
    if (imessageOsascriptCommandEnvName && env[imessageOsascriptCommandEnvName])
        merged.connectors.imessage.osascriptCommand = env[imessageOsascriptCommandEnvName];
    if (imessageChatDbPathEnvName && env[imessageChatDbPathEnvName])
        merged.connectors.imessage.chatDbPath = env[imessageChatDbPathEnvName];
    if (whatsappAccessTokenEnvName && env[whatsappAccessTokenEnvName])
        merged.connectors.whatsapp.accessToken = env[whatsappAccessTokenEnvName];
    if (whatsappPhoneNumberIdEnvName && env[whatsappPhoneNumberIdEnvName])
        merged.connectors.whatsapp.phoneNumberId = env[whatsappPhoneNumberIdEnvName];
    if (whatsappVerifyTokenEnvName && env[whatsappVerifyTokenEnvName])
        merged.connectors.whatsapp.verifyToken = env[whatsappVerifyTokenEnvName];
    if (whatsappGraphApiVersionEnvName && env[whatsappGraphApiVersionEnvName])
        merged.connectors.whatsapp.graphApiVersion = env[whatsappGraphApiVersionEnvName];
    if (lineChannelAccessTokenEnvName && env[lineChannelAccessTokenEnvName])
        merged.connectors.line.channelAccessToken = env[lineChannelAccessTokenEnvName];
    if (lineChannelSecretEnvName && env[lineChannelSecretEnvName])
        merged.connectors.line.channelSecret = env[lineChannelSecretEnvName];
    if (kakaotalkRequestTokenEnvName && env[kakaotalkRequestTokenEnvName])
        merged.connectors.kakaotalk.requestToken = env[kakaotalkRequestTokenEnvName];
    if (googleChatWebhookUrlEnvName && env[googleChatWebhookUrlEnvName])
        merged.connectors.googleChat.webhookUrl = env[googleChatWebhookUrlEnvName];
    ...
    if (genericWebhookUrlEnvName && env[genericWebhookUrlEnvName])
        merged.connectors.webhook.webhookUrl = env[genericWebhookUrlEnvName];
    ...
    if (genericWebhookInboundTokenEnvName && env[genericWebhookInboundTokenEnvName])
        merged.connectors.webhook.inboundToken = env[genericWebhookInboundTokenEnvName];
    if (genericWebhookInboundSignatureSecretEnvName && env[genericWebhookInboundSignatureSecretEnvName])
        merged.connectors.webhook.inboundSignatureSecret = env[genericWebhookInboundSignatureSecretEnvName];
    if (homeAssistantBaseUrlEnvName && env[homeAssistantBaseUrlEnvName])
        merged.connectors.homeAssistant.baseUrl = env[homeAssistantBaseUrlEnvName];
    if (homeAssistantAccessTokenEnvName && env[homeAssistantAccessTokenEnvName])
        merged.connectors.homeAssistant.accessToken = env[homeAssistantAccessTokenEnvName];
    if (homeAssistantServiceEnvName && env[homeAssistantServiceEnvName])
        merged.connectors.homeAssistant.service = env[homeAssistantServiceEnvName];
    ...
    if (teamsWebhookUrlEnvName && env[teamsWebhookUrlEnvName])
        merged.connectors.teams.webhookUrl = env[teamsWebhookUrlEnvName];
    ...
    if (mattermostWebhookUrlEnvName && env[mattermostWebhookUrlEnvName])
        merged.connectors.mattermost.webhookUrl = env[mattermostWebhookUrlEnvName];
    ...
    if (synologyChatWebhookUrlEnvName && env[synologyChatWebhookUrlEnvName])
        merged.connectors.synologyChat.webhookUrl = env[synologyChatWebhookUrlEnvName];
    ...
    if (rocketChatWebhookUrlEnvName && env[rocketChatWebhookUrlEnvName])
        merged.connectors.rocketChat.webhookUrl = env[rocketChatWebhookUrlEnvName];
    ...
    if (feishuWebhookUrlEnvName && env[feishuWebhookUrlEnvName])
        merged.connectors.feishu.webhookUrl = env[feishuWebhookUrlEnvName];
    ...
    if (dingTalkWebhookUrlEnvName && env[dingTalkWebhookUrlEnvName])
        merged.connectors.dingtalk.webhookUrl = env[dingTalkWebhookUrlEnvName];
    ...
    if (weComWebhookUrlEnvName && env[weComWebhookUrlEnvName])
        merged.connectors.wecom.webhookUrl = env[weComWebhookUrlEnvName];
    ...
    if (zaloAccessTokenEnvName && env[zaloAccessTokenEnvName])
        merged.connectors.zalo.accessToken = env[zaloAccessTokenEnvName];
    if (zaloRecipientEnvName && env[zaloRecipientEnvName])
        merged.connectors.zalo.recipient = env[zaloRecipientEnvName];
    ...
    if (ircHostEnvName && env[ircHostEnvName])
        merged.connectors.irc.host = env[ircHostEnvName];
    if (ircPortEnvName && env[ircPortEnvName])
        merged.connectors.irc.port = parseEnvInteger(env[ircPortEnvName], ircPortEnvName);
    if (ircTlsEnvName && env[ircTlsEnvName])
        merged.connectors.irc.tls = parseEnvBoolean(env[ircTlsEnvName], ircTlsEnvName);
    if (ircNickEnvName && env[ircNickEnvName])
        merged.connectors.irc.nick = env[ircNickEnvName];
    if (ircPasswordEnvName && env[ircPasswordEnvName])
        merged.connectors.irc.password = env[ircPasswordEnvName];
    if (ircChannelEnvName && env[ircChannelEnvName])
        merged.connectors.irc.channel = env[ircChannelEnvName];
    ...
    if (twitchAccessTokenEnvName && env[twitchAccessTokenEnvName])
        merged.connectors.twitch.accessToken = env[twitchAccessTokenEnvName];
    if (twitchBotUsernameEnvName && env[twitchBotUsernameEnvName])
        merged.connectors.twitch.botUsername = env[twitchBotUsernameEnvName];
    if (twitchChannelEnvName && env[twitchChannelEnvName])
        merged.connectors.twitch.channel = env[twitchChannelEnvName];
    ...
    if (ntfyBaseUrlEnvName && env[ntfyBaseUrlEnvName])
        merged.connectors.ntfy.baseUrl = env[ntfyBaseUrlEnvName];
    if (ntfyTokenEnvName && env[ntfyTokenEnvName])
        merged.connectors.ntfy.token = env[ntfyTokenEnvName];
    if (ntfyTopicEnvName && env[ntfyTopicEnvName])
        merged.connectors.ntfy.topic = env[ntfyTopicEnvName];
    ...
    if (mastodonBaseUrlEnvName && env[mastodonBaseUrlEnvName])
        merged.connectors.mastodon.baseUrl = env[mastodonBaseUrlEnvName];
    if (mastodonAccessTokenEnvName && env[mastodonAccessTokenEnvName])
        merged.connectors.mastodon.accessToken = env[mastodonAccessTokenEnvName];
    if (mastodonVisibilityEnvName && env[mastodonVisibilityEnvName])
        merged.connectors.mastodon.visibility = normalizeMastodonVisibility(env[mastodonVisibilityEnvName]);
    ...
    if (nextcloudTalkBaseUrlEnvName && env[nextcloudTalkBaseUrlEnvName])
        merged.connectors.nextcloudTalk.baseUrl = env[nextcloudTalkBaseUrlEnvName];
    if (nextcloudTalkUsernameEnvName && env[nextcloudTalkUsernameEnvName])
        merged.connectors.nextcloudTalk.username = env[nextcloudTalkUsernameEnvName];
    if (nextcloudTalkAppPasswordEnvName && env[nextcloudTalkAppPasswordEnvName])
        merged.connectors.nextcloudTalk.appPassword = env[nextcloudTalkAppPasswordEnvName];
    if (nextcloudTalkRoomTokenEnvName && env[nextcloudTalkRoomTokenEnvName])
        merged.connectors.nextcloudTalk.roomToken = env[nextcloudTalkRoomTokenEnvName];
    ...
    if (webexAccessTokenEnvName && env[webexAccessTokenEnvName])
        merged.connectors.webex.accessToken = env[webexAccessTokenEnvName];
    if (zulipSiteUrlEnvName && env[zulipSiteUrlEnvName])
        merged.connectors.zulip.siteUrl = env[zulipSiteUrlEnvName];
    if (zulipBotEmailEnvName && env[zulipBotEmailEnvName])
        merged.connectors.zulip.botEmail = env[zulipBotEmailEnvName];
    if (zulipApiKeyEnvName && env[zulipApiKeyEnvName])
        merged.connectors.zulip.apiKey = env[zulipApiKeyEnvName];
    if (zulipTargetEnvName && env[zulipTargetEnvName])
        merged.connectors.zulip.target = env[zulipTargetEnvName];
    ...
    if (emailSendmailCommandEnvName && env[emailSendmailCommandEnvName])
        merged.connectors.email.sendmailCommand = env[emailSendmailCommandEnvName];
    if (emailFromEnvName && env[emailFromEnvName])
        merged.connectors.email.from = env[emailFromEnvName];
    if (emailRecipientEnvName && env[emailRecipientEnvName])
        merged.connectors.email.recipient = env[emailRecipientEnvName];
    ...
    if (githubTokenEnvName && env[githubTokenEnvName])
        merged.connectors.github.token = env[githubTokenEnvName];
    if (githubTargetEnvName && env[githubTargetEnvName])
        merged.connectors.github.target = env[githubTargetEnvName];
    ...
    if (todoistTokenEnvName && env[todoistTokenEnvName])
        merged.connectors.todoist.token = env[todoistTokenEnvName];
    if (todoistProjectEnvName && env[todoistProjectEnvName])
        merged.connectors.todoist.project = env[todoistProjectEnvName];
    ...
    if (notionTokenEnvName && env[notionTokenEnvName])
        merged.connectors.notion.token = env[notionTokenEnvName];
    if (notionPageEnvName && env[notionPageEnvName])
        merged.connectors.notion.page = env[notionPageEnvName];
    ...
    if (obsidianVaultDirEnvName && env[obsidianVaultDirEnvName])
        merged.connectors.obsidian.vaultDir = env[obsidianVaultDirEnvName];
    if (obsidianNoteEnvName && env[obsidianNoteEnvName])
        merged.connectors.obsidian.note = env[obsidianNoteEnvName];
    ...
    for (const [id, provider] of Object.entries(merged.providers)) {
        ...
        if (provider.cwd)
            provider.cwd = resolve(baseDir, provider.cwd);
    }
    return merged;
}
function parseEnvInteger(value, name) {
...
export function findConfigPath(baseDir, explicitPath) {
    const envPath = env.VISER_CONFIG;
    if (explicitPath)
        return resolve(baseDir, explicitPath);
    if (envPath)
