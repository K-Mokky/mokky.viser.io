...
import { localSmoke } from "./smoke.js";
import { providerIssueAdvice } from "../providers/guide.js";
export async function verify(config, options = {}) {
...
function providerRecoveryAdvice(readiness, providers) {
    return readiness
...
function providerIdFromProbeMessage(message) {
    return /^([^:]+):/.exec(message)?.[1];
}
function recommendedCommands(readiness, audit) {
