...
import { assertNoSymlinkComponentsUnderRoot, ensurePrivateDir, fileExists, readPrivateFileIfExists, writePrivateFile } from "../utils/files.js";
import { normalizeClipboardText, normalizeExternalUrl, normalizeSpeechText, parseBrowserTaskContent, parseCalendarEventContent, parseConnectorMessageContent, parseDesktopNotificationContent, parseMailDraftContent } from "../core/actions.js";
import { normalizePersonalizationState } from "../core/personalization.js";
...
export async function stateHealth(config, options = {}) {
...
export async function stateHealthReport(config, options = {}) {
    return (await stateHealth(config, options)).report;
}
export async function stateHealthItems(config) {
    const items = [];
    if (!await isSafeStorageRoot(config.storage.dir, config.assistant.workdir, items))
        return items;
...
export function summarizeStateHealth(items) {
...
async function checkSessionFiles(storageDir, workdir, items) {
    const dir = join(storageDir, "sessions");
    if (!await isRegularStateDirectory("sessions", dir, workdir, items))
        return;
    const names = (await readdir(dir)).filter((name) => name.endsWith(".jsonl")).sort();
    for (const name of names)
        await checkJsonlFile("sessions", join(dir, name), workdir, items, { sessionFile: true });
}
async function checkJsonFile(area, path, kind, workdir, items) {
    if (!fileExists(path))
        return;
    if (!await isRegularStateFile(area, path, workdir, items))
        return;
    try {
...
async function checkJsonlFile(area, path, workdir, items, options = {}) {
    if (!fileExists(path))
        return;
    if (!await isRegularStateFile(area, path, workdir, items))
        return;
...
function largeSessionWarning(path, raw, validLines) {
...
async function isRegularStateFile(area, path, workdir, items) {
    if (!await isRegularStateDirectory(area, dirname(path), workdir, items))
        return false;
    try {
...
async function isSafeStorageRoot(storageDir, workdir, items) {
    try {
...
async function isRegularStateDirectory(area, path, workdir, items) {
    try {
        const info = await lstat(path);
        if (info.isSymbolicLink()) {
...
function pushUniqueStateItem(items, item) {
    if (items.some((existing) => existing.area === item.area && existing.path === item.path && existing.message === item.message))
        return;
    items.push(item);
}
function validateJsonShape(value, kind) {
    if (kind === "json-array")
        return Array.isArray(value) ? { ok: true } : { ok: false, reason: "expected a JSON array" };
    if (kind === "json-object")
        return isPlainObject(value) ? { ok: true } : { ok: false, reason: "expected a JSON object" };
    if (kind === "scheduler-tasks")
        return validateArrayItems(value, "scheduled task", validateScheduledTask);
    if (kind === "jobs-state")
        return validateArrayItems(value, "queued job", validateQueuedJob);
    if (kind === "actions-state")
        return validateArrayItems(value, "pending action", validatePendingAction);
    if (kind === "personalization-state")
        return validatePersonalizationState(value);
    return validateAccessState(value);
}
function validateArrayItems(value, label, validate) {
    if (!Array.isArray(value))
        return { ok: false, reason: "expected a JSON array" };
    for (const [index, item] of value.entries()) {
        const problem = validate(item);
        if (problem)
            return { ok: false, reason: `${label} at index ${index}: ${problem}` };
    }
    return { ok: true };
}
function validateScheduledTask(value) {
    if (!isPlainObject(value))
        return "expected object";
    const requiredString = firstMissingString(value, ["id", "prompt", "sessionId", "source", "createdAt"]);
    if (requiredString)
        return `${requiredString} must be a string`;
    if (!VALID_TASK_SOURCES.includes(String(value.source)))
        return `source must be one of ${VALID_TASK_SOURCE_LABEL}`;
    if (typeof value.enabled !== "boolean")
        return "enabled must be a boolean";
    if (!Number.isInteger(value.runCount) || Number(value.runCount) < 0)
        return "runCount must be a non-negative integer";
    if (value.providerId !== undefined && typeof value.providerId !== "string")
        return "providerId must be a string when present";
    if (value.nextRunAt !== undefined && typeof value.nextRunAt !== "string")
        return "nextRunAt must be a string when present";
    if (value.lastRunAt !== undefined && typeof value.lastRunAt !== "string")
        return "lastRunAt must be a string when present";
    if (value.lastError !== undefined && typeof value.lastError !== "string")
        return "lastError must be a string when present";
...
function validateQueuedJob(value) {
    if (!isPlainObject(value))
        return "expected object";
    ...
    if (!VALID_TASK_SOURCES.includes(String(value.source)))
        return `source must be one of ${VALID_TASK_SOURCE_LABEL}`;
    if (!["pending", "running", "done", "failed", "cancelled"].includes(String(value.status)))
        return "status must be pending, running, done, failed, or cancelled";
    ...
    if (value.dependsOn !== undefined) {
        if (!Array.isArray(value.dependsOn))
            return "dependsOn must be an array when present";
        if (value.dependsOn.length > 20)
            return "dependsOn must contain at most 20 ids";
        for (const [index, id] of value.dependsOn.entries()) {
            if (typeof id !== "string" || !id.trim())
                return `dependsOn[${index}] must be a non-empty string`;
        }
    }
...
function validatePendingAction(value) {
    ...
    if (requiredString)
        return `${requiredString} must be a string`;
    if (!["write-file", "append-file", "open-url", "speak", "calendar-event", "mail-draft", "notify", "clipboard", "connector-message", "browser-task"].includes(String(value.type)))
        return "type must be write-file, append-file, open-url, speak, calendar-event, mail-draft, notify, clipboard, connector-message, or browser-task";
    ...
    if (value.type === "speak") {
        if (value.targetPath !== "local-tts")
            return "speak targetPath must be local-tts";
    ...
    if (value.type === "calendar-event") {
        if (!String(value.targetPath).endsWith(".ics"))
            return "calendar-event targetPath must be an .ics file";
        if (!redactedDecidedContent) {
            try {
    ...
    if (value.type === "mail-draft") {
        if (!String(value.targetPath).startsWith("mailto:"))
            return "mail-draft targetPath must be a mailto URL";
        try {
            normalizeExternalUrl(String(value.targetPath));
        }
        catch (error) {
    ...
    if (value.type === "notify") {
        if (value.targetPath !== "local-notification")
            return "notify targetPath must be local-notification";
        if (!redactedDecidedContent) {
            try {
    ...
    if (value.type === "clipboard") {
        if (value.targetPath !== "local-clipboard")
            return "clipboard targetPath must be local-clipboard";
        if (!redactedDecidedContent) {
            try {
    ...
    if (value.type === "connector-message") {
        ...
        if (!redactedDecidedContent) {
            try {
                ...
                if (`${message.connector}:${message.targetId}` !== String(value.targetPath))
                    return "connector-message targetPath must match content connector and targetId";
            }
            catch (error) {
                return error instanceof Error ? error.message : String(error);
            }
    ...
    if (value.type === "browser-task") {
        if (!/^(?:browser-use-cloud|local-cdp|browserbase-session|firecrawl-interact):/u.test(String(value.targetPath)))
            return "browser-task targetPath must start with browser-use-cloud:, local-cdp:, browserbase-session:, or firecrawl-interact:";
        if (!redactedDecidedContent) {
            try {
                const task = parseBrowserTaskContent(String(value.content));
                if (`${task.provider}:${task.allowedDomains.join(",")}` !== String(value.targetPath))
                    return "browser-task targetPath must match content provider and allowed domains";
            }
            catch (error) {
                return error instanceof Error ? error.message : String(error);
            }
    ...
    if (value.decidedAt !== undefined && typeof value.decidedAt !== "string")
        return "decidedAt must be a string when present";
    if (value.backupPath !== undefined && typeof value.backupPath !== "string")
        return "backupPath must be a string when present";
    return undefined;
}
function validatePersonalizationState(value) {
    if (!isPlainObject(value))
        return { ok: false, reason: "expected personalization state object" };
    if (value.version !== undefined && value.version !== 1)
        return { ok: false, reason: "personalization version must be 1 when present" };
    ...
    if (!Array.isArray(value.settings))
        return { ok: false, reason: "expected personalization state with settings[]" };
    try {
    ...
    for (const [index, setting] of value.settings.entries()) {
        if (!isPlainObject(setting))
            return { ok: false, reason: `personalization setting at index ${index}: expected object` };
        const missing = firstMissingString(setting, ["key", "value", "source", "createdAt", "updatedAt"]);
        if (missing)
            return { ok: false, reason: `personalization setting at index ${index}: ${missing} must be a string` };
    }
    return { ok: true };
}
function validateAccessState(value) {
    if (!isPlainObject(value))
        return { ok: false, reason: "expected access state object" };
    ...
    for (const [index, peer] of value.peers.entries()) {
        ...
        if (problem)
            return { ok: false, reason: `access peer at index ${index}: ${problem}` };
    }
    for (const [index, code] of value.codes.entries()) {
        const problem = validatePairingCode(code);
        if (problem)
            return { ok: false, reason: `pairing code at index ${index}: ${problem}` };
    }
    return { ok: true };
}
function validateAccessPeer(value) {
    if (!isPlainObject(value))
        return "expected object";
    const requiredString = firstMissingString(value, ["connector", "id", "createdAt", "source"]);
    if (requiredString)
        return `${requiredString} must be a string`;
    if (!VALID_ACCESS_CONNECTORS.includes(String(value.connector)))
        return `connector must be one of ${VALID_ACCESS_CONNECTOR_LABEL}`;
...
function validatePairingCode(value) {
    ...
    if (requiredString)
        return `${requiredString} must be a string`;
    if (value.connector !== undefined && !VALID_ACCESS_CONNECTORS.includes(String(value.connector)))
        return `connector must be one of ${VALID_ACCESS_CONNECTOR_LABEL} when present`;
    if (value.label !== undefined && typeof value.label !== "string")
        return "label must be a string when present";
    if (value.usedAt !== undefined && typeof value.usedAt !== "string")
        return "usedAt must be a string when present";
    return undefined;
}
export async function repairState(config, items, options) {
    const plans = items
...
function repairedContent(plan) {
    if (plan.replacementContent !== undefined)
        return plan.replacementContent;
    if (plan.kind === "jsonl") {
        const lines = plan.validJsonlLines ?? [];
        return lines.length ? `${lines.join("\n")}\n` : "";
    }
    if (["json-array", "scheduler-tasks", "jobs-state", "actions-state"].includes(plan.kind))
        return "[]\n";
    if (plan.kind === "access-state")
        return `${JSON.stringify({ peers: [], codes: [] }, null, 2)}\n`;
    if (plan.kind === "personalization-state")
        return `${JSON.stringify({ version: 1, settings: [] }, null, 2)}\n`;
    return "{}\n";
}
function formatStateHealthReport(items, summary, repairs, repairRequested, dryRun) {
    return [
...
async function stateFileItem(area, path, message, workdir) {
    try {
        await assertNoSymlinkComponentsUnderRoot(path, workdir);
        const mode = (await lstat(path)).mode & 0o777;
        if ((mode & 0o077) === 0)
            return { status: "pass", area, path, message };
        return {
            status: "warn",
            area,
            path,
...
async function readStateFileIfStillSafe(area, path, workdir, items) {
    try {
        await assertNoSymlinkComponentsUnderRoot(path, workdir);
...
async function readRepairSourceFile(path, workdir) {
    await assertNoSymlinkComponentsUnderRoot(path, workdir);
    const raw = await readPrivateFileIfExists(path, { dirs: [dirname(path)] });
    if (raw === undefined)
        throw new Error(`State repair source disappeared before backup: ${path}`);
    return raw;
}
...
function cleanAccessState(value) {
    ...
    const peers = value.peers.map((peer) => {
        if (!isPlainObject(peer) || typeof peer.source !== "string" || !/^pair:[A-F0-9]{8}$/iu.test(peer.source))
            return peer;
    ...
    const codes = value.codes.filter((code) => {
        if (!isPlainObject(code))
            return true;
        if (typeof code.usedAt === "string" && code.usedAt.trim())
            return false;
...
function cleanActionsState(value) {
    if (!Array.isArray(value))
        return { changed: false, state: [], redactedCount: 0 };
    let redactedCount = 0;
    const actions = value.map((action) => {
        ...
        if (typeof action.content !== "string")
            return action;
        if (/^\[\d+ bytes\]$/u.test(action.content))
            return action;
        redactedCount += 1;
...
function firstMissingString(value, keys) {
