...
import { splitCommandLine } from "../core/tools.js";
import { runCommand } from "../utils/exec.js";
import { listPrivateDirIfExists, readPrivateFileIfExists, writePrivateFile } from "../utils/files.js";
...
export async function benchmarkReport(config, options = {}) {
...
export async function latestBenchmarkArtifactSummary(config) {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            if (parsed.schemaVersion !== 1 || !parsed.generatedAt || !parsed.viser)
                continue;
...
async function runViserProviderBenchmark(assistant, providerId, prompt, iterations, warmup) {
...
async function runBaselineBenchmark(spec, prompt, iterations, warmup, timeoutMs, cwd) {
    for (let index = 0; index < warmup; index += 1) {
...
async function runBaselineOnce(spec, prompt, timeoutMs, cwd) {
...
function benchmarkRun(label, samples) {
    return {
        label,
...
function compareRuns(viser, baselines) {
    if (!viser.ok)
        return "viser-failed";
    if (baselines.length === 0)
        return "no-baseline";
    if (baselines.some((baseline) => !baseline.ok))
        return "baseline-failed";
    return baselines.every((baseline) => viser.stats.medianMs <= baseline.stats.medianMs)
        ? "viser-not-slower"
        : "baseline-faster";
}
function parseBaselineSpecs(options) {
    const specs = [
    ...
    return specs.filter((spec) => Boolean(spec));
}
function parseBaselineSpec(defaultLabel, raw) {
    ...
    if (!command)
        throw new Error(`Benchmark baseline '${label}' is missing a command.`);
    return { label, command, args };
}
function benchmarkConfigFor(config, artifactDir, options) {
...
async function saveBenchmarkArtifact(config, result, artifactPath) {
...
function benchmarkNextSteps(live, baselineCount) {
    const lines = [];
    if (!live) {
