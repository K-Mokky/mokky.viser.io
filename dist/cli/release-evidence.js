...
import { latestBenchmarkArtifactSummary } from "./benchmark.js";
import { CORE_LOCAL_CLI_ROUTES, commandBasename, coreLocalCliRoutePass } from "../core/local-cli-policy.js";
...
export async function releaseEvidence(config, options = {}) {
...
export async function releaseEvidenceReport(config, options = {}) {
    return (await releaseEvidenceReportResult(config, options)).report;
}
export async function releaseEvidenceReportResult(config, options = {}) {
    const rootDir = options.rootDir ?? cwd();
...
async function packageReleaseChecks(rootDir, packageJson) {
    const checks = [];
    if (!packageJson) {
...
async function securityDocumentChecks(rootDir) {
    const checks = [];
...
async function githubTemplateChecks(rootDir) {
    const checks = [];
...
async function contributingDocumentChecks(rootDir) {
    const checks = [];
...
async function privacyDocumentChecks(rootDir) {
    const checks = [];
    ...
    addCheck(checks, "privacy-doc", /Viser/iu.test(normalized) && /KMokky/iu.test(normalized) && /creator|credit|제작자/iu.test(normalized), "PRIVACY.md limits public identity to Viser and KMokky creator credit");
    addCheck(checks, "privacy-doc", /\.viser|\.omx|session|memory|runtime state|backups/iu.test(normalized), "PRIVACY.md identifies local runtime state that must stay private");
    addCheck(checks, "privacy-doc", /TELEGRAM_BOT_TOKEN|DISCORD_BOT_TOKEN|SIGNAL_CLI_ACCOUNT|WHATSAPP_ACCESS_TOKEN|WHATSAPP_VERIFY_TOKEN|LINE_CHANNEL_ACCESS_TOKEN|LINE_CHANNEL_SECRET|KAKAOTALK_SKILL_TOKEN|WEBEX_ACCESS_TOKEN|ZULIP_API_KEY|personal handles|email addresses|local filesystem paths|\.env/iu.test(normalized), "PRIVACY.md tells users not to publish tokens, handles, emails, or local paths");
    return checks;
}
async function publicExampleChecks(rootDir) {
    const checks = [];
    ...
    addCheck(checks, "public-example", modelApiKeyReferences(envExample).length === 0, ".env.example excludes model API key variables");
    ...
    addCheck(checks, "public-example", modelApiKeyEnvKeysFromConfigExample(configExample).length === 0, "config example provider env excludes model API key variables");
    return checks;
}
async function objectiveCoverageChecks(rootDir, config) {
    const checks = [];
    ...
    for (const route of CORE_LOCAL_CLI_ROUTES) {
        addCheck(checks, "objective", coreLocalCliRoutePass(config, route), `${route.label} route uses exact logged-in local ${route.expectedCommand} CLI provider`);
    }
...
function formatReleaseEvidence(result) {
    const blockers = [
...
function addCheck(checks, area, ok, message) {
    checks.push({ status: ok ? "pass" : "fail", area, message });
}
function completionAudit(publicReleaseReady, objectives) {
...
async function latestApprovedRealProviderSkillReflectionProof(config) {
    ...
    for (const skillDir of config.skills.dirs) {
        ...
        for (const line of raw.split("\n")) {
            ...
            if (!providerId || !isRealReflectionProvider(providerId, config))
                continue;
            if (!actionId || actions.get(actionId) !== "approved")
                continue;
            if (!isSafeRelativeSkillTarget(target))
                continue;
            if (transcriptMessages === undefined || procedureBytes === undefined)
                continue;
            if (!await fileExists(join(skillDir, target)))
                continue;
            const summary = {
                providerId,
            ...
            if (!latest || summary.createdAt.localeCompare(latest.createdAt) >= 0)
                latest = summary;
        }
    }
    return latest;
}
async function readActionStatusMap(path) {
...
async function latestApprovedBrowserTaskProofs(config) {
    const actions = await readActionStatusMap(join(config.actions.dir, "actions.json"));
    ...
    for (const line of raw.split("\n")) {
        ...
        const createdAt = stringValue(proof.createdAt);
        if (!isBrowserTaskProviderValue(provider))
            continue;
        if (!actionId || actions.get(actionId) !== "approved")
            continue;
        if (!resultId || titleBytes === undefined || textBytes === undefined)
            continue;
        const summary = {
            provider,
            resultId,
...
function isRealReflectionProvider(providerId, config) {
    if (!config.providers[providerId])
        return false;
    return !/^(?:echo|fake|mock|smoke|test)$/iu.test(providerId);
}
function isSafeRelativeSkillTarget(target) {
...
function isBrowserTaskProviderValue(value) {
    return value === "browser-use-cloud" || value === "local-cdp" || value === "browserbase-session" || value === "firecrawl-interact";
}
function browserTaskLiveProofGaps(proofs) {
    const proven = new Set(proofs.map((proof) => proof.provider));
    const missingRemote = [];
    if (!proven.has("browser-use-cloud"))
        missingRemote.push("Browser Use");
    if (!proven.has("browserbase-session"))
        missingRemote.push("Browserbase");
    if (!proven.has("firecrawl-interact"))
        missingRemote.push("Firecrawl");
    const missing = [];
    if (missingRemote.length > 0)
        missing.push(`${formatEnglishList(missingRemote)} API credential/task proof`);
    if (!proven.has("local-cdp"))
        missing.push("a real localhost CDP browser run");
...
function providerProbeMode(proof) {
    if (proof.probeAllProviders)
        return "all providers";
    if (proof.probeProviders)
        return "launch route";
    return "not requested";
}
function proofChecks(readinessItems) {
...
function safeProofMessage(area, status, message) {
    ...
    function visit(current) {
        ...
        for (const [key, nested] of Object.entries(current)) {
            if (key === "env" && isRecord(nested)) {
                for (const envKey of Object.keys(nested))
                    keys.add(envKey);
            }
            visit(nested);
        }
...
function redactReleaseEvidence(text, rootDir, config) {
    let output = text;
    const replacements = [
        [rootDir, "<workspace>"],
