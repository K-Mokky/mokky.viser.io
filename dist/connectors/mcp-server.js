...
export class ViserMcpServer {
    ...
    async readResource(params) {
        ...
        if (uri === "viser://status")
            return resourceText(uri, "text/plain", await this.assistant.status("mcp:resource"));
        const fileResource = PUBLIC_RESOURCE_FILES[uri];
        if (fileResource)
            return resourceText(uri, fileResource.mimeType, await readPublicWorkspaceFile(this.config, fileResource.path));
...
export async function runMcpStdioServer(config, assistant, io = {}) {
...
async function readPublicWorkspaceFile(config, relativePath) {
    const path = join(config.assistant.workdir, relativePath);
    const info = await lstat(path);
    if (info.isSymbolicLink())
        throw new Error(`MCP public resource is a symlink; refusing to read: ${relativePath}`);
    if (!info.isFile())
        throw new Error(`MCP public resource is not a file: ${relativePath}`);
...
function requiredArg(args, key) {
    ...
    if (!value)
        throw new Error(`Missing required MCP tool argument: ${key}`);
    return value;
}
function mcpCommandToken(value, key) {
    if (/[\s'"\u0000-\u001f\u007f]/u.test(value)) {
        throw new Error(`MCP tool argument '${key}' must not contain whitespace, quotes, or control characters.`);
    }
    return value;
}
function mcpQuotedCommandToken(value, key) {
    if (/['"\u0000-\u001f\u007f]/u.test(value)) {
        throw new Error(`MCP tool argument '${key}' must not contain quotes or control characters.`);
    }
    return /\s/u.test(value) ? `"${value}"` : value;
}
function optionalPositiveIntegerToken(args, key) {
    const value = argString(args, key);
    if (!value)
        return "";
    if (!/^[1-9]\d{0,8}$/u.test(value))
        throw new Error(`MCP tool argument '${key}' must be a positive integer string.`);
    return value;
}
function optionalEnumToken(args, key, values) {
    const value = argString(args, key);
    if (!value)
        return "";
    if (!values.includes(value))
        throw new Error(`MCP tool argument '${key}' must be one of: ${values.join(", ")}.`);
    return value;
}
