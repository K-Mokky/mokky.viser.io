...
export async function sendGoogleChatMessage(config, webhookId, text, options = {}) {
...
export function resolveGoogleChatWebhookUrl(config, webhookId) {
    const id = normalizeWebhookId(webhookId);
    if (!id)
        throw new Error("Google Chat webhook id must be a short alias such as default or ops.");
...
export function parseWebhookUrlMap(value) {
...
export function normalizeGoogleChatWebhookUrl(value) {
    const trimmed = value.trim();
    let url;
    try {
        url = new URL(trimmed);
...
function redactGoogleChatDetail(detail, config, webhookId, text) {
    let output = detail;
