...
import { genericWebhookInboundBodyLimitBytes, handleGenericWebhookInbound, normalizeGenericWebhookInboundPath, normalizeGenericWebhookInboundToken, redactGenericWebhookDetail } from "./generic-webhook.js";
...
export function createWebDashboardState(options = {}) {
    return {
...
export async function createPersistentWebDashboardState(canvasDir, options = {}) {
...
export async function startWebDashboard(assistant, options) {
...
export async function handleWebDashboardRequest(request, response, assistant, sessionId, state = createWebDashboardState(), genericWebhook) {
...
async function handleDashboardLogin(request, response, method, state) {
    if (!state.authToken) {
...
function sendUnauthorized(response, method, state) {
    if (!state.authToken)
        return;
...
function isDashboardAuthorized(request, state) {
    if (!state.authToken)
        return true;
    const authorization = requestHeader(request, "authorization");
    const bearer = authorization?.match(/^Bearer\s+(.+)$/iu)?.[1]?.trim();
    if (bearer && safeTokenEquals(bearer, state.authToken))
        return true;
...
function parseDashboardLoginToken(body, contentType) {
...
function dashboardSessionCookie(sessionToken, request) {
...
function verifyGenericWebhookInboundSignature(config, body, request) {
    ...
    if (Math.abs(Date.now() - timestamp.timeMs) > toleranceMs)
        return "Forbidden";
...
function normalizeWebhookSignatureTimestamp(value) {
...
function normalizeDashboardAuthToken(value) {
    const token = typeof value === "string" ? value.trim() : "";
    return token || undefined;
}
async function sendDashboardEvents(request, response, method, assistant, sessionId, intervalMs) {
...
async function sendCanvasEvents(request, response, method, state, intervalMs) {
    response.statusCode = 200;
    response.setHeader("content-type", "text/event-stream; charset=utf-8");
    response.setHeader("cache-control", "no-store");
    response.setHeader("connection", "keep-alive");
...
async function handleCanvasPost(request, response, state) {
...
async function handleWebChatPost(request, response, assistant, sessionId, state) {
    if (!isLocalDashboardRequest(request)) {
...
async function handleGenericWebhookPost(request, response, assistant, config) {
    let token;
    try {
...
function isLocalDashboardRequest(request) {
...
function normalizeWebChatMessage(value) {
    const text = typeof value === "string" ? value.replace(/\r\n?/gu, "\n").trim() : "";
    if (!text)
        throw new Error("message is required");
    if (text.length > MAX_WEB_CHAT_MESSAGE_CHARS)
        throw new Error(`message is too long (${text.length} > ${MAX_WEB_CHAT_MESSAGE_CHARS})`);
    if (/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/u.test(text))
        throw new Error("message contains control characters");
    return text;
}
function normalizeOptionalWebChatProvider(value) {
...
async function commitCanvasPatch(state, patch) {
...
function applyCanvasPatch(strokes, patch) {
    if (patch.action === "clear")
        return [];
    return [...strokes, patch.stroke].slice(-MAX_CANVAS_STROKES);
}
async function persistCanvasState(state) {
    if (!state.canvasStorePath)
        return;
    try {
...
async function loadPersistedCanvasStrokes(canvasStorePath) {
    ...
    const strokesRaw = Array.isArray(record?.strokes)
        ...
        : Array.isArray(parsed)
            ...
            : undefined;
    if (!strokesRaw)
        throw new Error("Canvas store must contain a strokes array.");
    return strokesRaw
        .map(normalizeStoredCanvasStroke)
        .filter((stroke) => Boolean(stroke))
        .slice(-MAX_CANVAS_STROKES);
}
function normalizeStoredCanvasStroke(value) {
    const input = asRecord(value);
    const pointsRaw = Array.isArray(input?.points) ? input.points : [];
    if (!input || pointsRaw.length < 2)
        return undefined;
    try {
        return {
...
function normalizeCanvasPatch(value) {
    const input = asRecord(value);
    if (!input)
        throw new Error("Canvas patch must be an object.");
    if (input.action === "clear")
        return { action: "clear" };
    const pointsRaw = Array.isArray(input.points) ? input.points : [];
    if (pointsRaw.length < 2)
        throw new Error("Canvas stroke requires at least two points.");
    if (pointsRaw.length > MAX_CANVAS_POINTS)
        throw new Error(`Canvas stroke has too many points (${pointsRaw.length} > ${MAX_CANVAS_POINTS}).`);
    return {
        action: "stroke",
        stroke: {
...
function normalizeCanvasPoint(value) {
    const point = asRecord(value);
    const x = typeof point?.x === "number" ? point.x : Number.NaN;
    const y = typeof point?.y === "number" ? point.y : Number.NaN;
    if (!Number.isFinite(x) || !Number.isFinite(y))
        throw new Error("Canvas point coordinates must be numbers.");
    return {
        x: Math.max(0, Math.min(1, x)),
        y: Math.max(0, Math.min(1, y))
    };
}
function normalizeCanvasUser(value) {
...
function setSecurityHeaders(response) {
    ...
    response.setHeader("content-security-policy", "default-src 'none'; connect-src 'self'; img-src 'self' data: blob:; media-src 'self' blob:; style-src 'unsafe-inline'; script-src 'unsafe-inline'; form-action 'self'; base-uri 'none'; frame-ancestors 'none'");
}
...
function collaborativeCanvasHtml(token, persistent) {
...
export function dashboardCanvasSvg(data) {
    const width = 1000;
    const height = 360;
...
function svgNode(x, y, label, value, color) {
    return [
        `  <g filter="url(#shadow)">`,
...
function svgHub(x, y, title, lines) {
    return [
        `  <g filter="url(#shadow)">`,
