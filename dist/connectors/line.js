...
export async function runLineBridge(config, assistant, access) {
...
export async function startLineWebhookServer(config, assistant, access, rateLimiter = new ConnectorRateLimiter(config.maxMessagesPerMinute), options = {}) {
...
export async function handleLineHttpRequest(request, response, config, assistant, access, rateLimiter, options = {}) {
...
export function verifyLineSignature(config, body, signature) {
...
export async function handleLineWebhookPayload(config, assistant, payload, access, rateLimiter, options = {}) {
...
export async function handleLineMessage(config, assistant, message, access, rateLimiter, options = {}) {
    const peerId = normalizeLinePeerId(message.peerId);
    if (!peerId || !message.text)
        return;
    const staticAllowlist = [...config.allowedPeerIds, ...config.defaultPeerIds].map(normalizeLinePeerId).filter(Boolean);
    if (config.allowedPeerIds.length > 0 && !staticAllowlist.includes(peerId))
        return;
...
export async function sendLinePushMessage(config, peerId, text, options = {}) {
    ...
    if (!safePeerId)
        throw new Error("LINE peer ID must be a userId, groupId, or roomId alias value.");
    await sendLineApiMessages(config, LINE_PUSH_URL, { to: safePeerId }, text, options, safePeerId);
}
export async function sendLineReplyMessage(config, replyToken, text, options = {}) {
...
export function parseLineWebhookMessages(payload) {
...
export function normalizeLineInput(content) {
    const trimmed = content.trim();
    return trimmed || undefined;
}
export function normalizeLinePeerId(value) {
...
export function redactLineDetail(detail, config, peerId, text) {
    let output = detail;
...
async function sendLineResponse(config, peerId, replyToken, text, options) {
    if (replyToken) {
...
async function sendLineApiMessages(config, url, basePayload, text, options, redactionTarget) {
    if (!config.channelAccessToken)
        throw new Error(`LINE channel access token is missing. Set ${config.channelAccessTokenEnv}.`);
...
function normalizeLineReplyToken(value) {
    const trimmed = value?.trim();
...
async function readRequestBody(request, maxBytes) {
    ...
    for await (const chunk of request) {
        ...
        if (bytes > maxBytes)
            throw new Error("LINE webhook body is too large.");
        body += piece;
    }
    return body;
}
