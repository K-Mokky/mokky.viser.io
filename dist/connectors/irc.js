...
export async function sendIrcMessage(config, channelId, text, options = {}) {
...
export function resolveIrcChannel(config, channelId) {
...
export function hasIrcChannel(config) {
    return Boolean(config.channel || Object.keys(config.channels).length > 0);
}
export function parseIrcChannelMap(raw) {
    const output = {};
...
export function normalizeIrcChannelAlias(value) {
    const id = normalizeWebhookId(value);
    if (!id)
        throw new Error("IRC channel alias must be a short alias such as default or ops.");
    return id.toLowerCase();
}
export function normalizeIrcHost(value) {
...
export function normalizeIrcPort(value) {
    if (!Number.isInteger(value) || value < 1 || value > 65_535) {
        throw new Error("IRC port must be an integer between 1 and 65535.");
    }
    return value;
}
export function normalizeIrcNick(value) {
...
export function normalizeIrcPassword(value) {
...
export function normalizeIrcChannel(value) {
...
export function redactIrcDetail(detail, config, alias, channel, text) {
    let output = detail;
...
async function runIrcSend(input) {
    await new Promise((resolve, reject) => {
        ...
        socket.on("end", () => {
            if (!registered)
                fail(new Error("server closed before registration"));
        });
    });
}
function createIrcSocket(input) {
...
function normalizeIrcMessageBody(raw) {
    if (/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/u.test(raw))
        throw new Error("IRC message contains control characters.");
    const text = raw.replace(/\r\n?/gu, "\n").split("\n").map((line) => line.trim()).filter(Boolean).join(" / ").trim();
    if (!text)
        throw new Error("IRC message is required.");
    if (text.length > 20_000)
        throw new Error("IRC message is too long.");
    return text;
}
