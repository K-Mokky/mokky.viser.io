...
export async function sendNextcloudTalkMessage(config, roomId, text, options = {}) {
...
export function resolveNextcloudTalkRoom(config, roomId) {
...
export function hasNextcloudTalkRoom(config) {
    return Boolean(config.roomToken || Object.keys(config.rooms).length > 0);
}
export function parseNextcloudTalkRoomMap(raw) {
    const output = {};
...
export function normalizeNextcloudTalkRoomAlias(value) {
    const id = normalizeWebhookId(value);
    if (!id)
        throw new Error("Nextcloud Talk room alias must be a short alias such as default or ops.");
    return id.toLowerCase();
}
export function normalizeNextcloudTalkBaseUrl(value) {
    const raw = value?.trim() ?? "";
    if (!raw)
        throw new Error("Nextcloud Talk base URL is required.");
    let url;
    try {
        url = new URL(raw);
    }
    ...
    if (url.protocol !== "https:")
        throw new Error("Nextcloud Talk base URL must use https.");
    if (url.username || url.password)
        throw new Error("Nextcloud Talk base URL must not include credentials.");
    if (url.search || url.hash)
        throw new Error("Nextcloud Talk base URL must not include query strings or fragments.");
    url.pathname = url.pathname.replace(/\/+$/u, "");
    return url.toString().replace(/\/$/u, "");
}
export function normalizeNextcloudTalkUsername(value) {
    const username = value?.trim() ?? "";
    if (!username)
        throw new Error("Nextcloud Talk username is required.");
...
export function normalizeNextcloudTalkAppPassword(value) {
    const password = value?.trim() ?? "";
    if (!password)
        throw new Error("Nextcloud Talk app password is required.");
    if (password.length > 512 || /[\r\n\x00]/u.test(password)) {
        throw new Error("Nextcloud Talk app password must be a single opaque line.");
    }
    return password;
}
export function normalizeNextcloudTalkRoomToken(value) {
    const token = value?.trim() ?? "";
    if (!token)
        throw new Error("Nextcloud Talk room token is required.");
...
export function nextcloudTalkAuthorizationHeader(username, appPassword) {
    return `Basic ${Buffer.from(`${username}:${appPassword}`, "utf8").toString("base64")}`;
}
export function redactNextcloudTalkDetail(detail, config, alias, roomToken, text) {
    let output = detail;
...
function normalizeNextcloudTalkMessageBody(raw) {
    if (/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/u.test(raw))
        throw new Error("Nextcloud Talk message contains control characters.");
    const text = raw.replace(/\r\n?/gu, "\n").trim();
    if (!text)
        throw new Error("Nextcloud Talk message is required.");
    if (text.length > 20_000)
        throw new Error("Nextcloud Talk message is too long.");
    return text;
}
function nextcloudTalkJsonFailed(bodyText) {
    if (!bodyText.trim())
        return false;
    try {
        ...
        if (!meta)
            return false;
        if (typeof meta.status === "string" && meta.status.toLowerCase() === "failure")
            return true;
        if (typeof meta.statuscode === "number" && meta.statuscode >= 400)
            return true;
        return false;
    }
    catch {
