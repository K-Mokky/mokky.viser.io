...
export async function sendGitHubIssueComment(config, targetId, text, options = {}) {
...
export function resolveGitHubIssueTarget(config, targetId) {
...
export function hasGitHubTarget(config) {
    return Boolean(config.target || Object.keys(config.targets).length > 0);
}
export function hasGitHubCredentials(config) {
...
export function normalizeGitHubTargetAlias(value) {
    const trimmed = value?.trim();
...
export function parseGitHubIssueTarget(value) {
    const trimmed = value.trim();
    if (!trimmed)
        throw new Error("GitHub issue target is required.");
...
export function parseGitHubIssueTargetMap(value) {
...
export function formatGitHubIssueTarget(target) {
...
export function redactGitHubDetail(detail, config, alias, target, text) {
    let output = detail;
...
function normalizeGitHubIssueTarget(owner, repo, issueNumber) {
...
function normalizeGitHubPathPart(value, label) {
