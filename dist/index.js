...
import { auditReport } from "./cli/audit.js";
import { benchmarkReport } from "./cli/benchmark.js";
import { createBackupReport } from "./cli/backup.js";
import { compactBackupReport } from "./cli/compact-backups.js";
import { configCheckReport } from "./cli/config-check.js";
import { dashboardCheck } from "./cli/dashboard-check.js";
import { doctorReport } from "./cli/doctor.js";
import { envCheckReport, writeEnvTemplate } from "./cli/env-check.js";
import { writeExampleConfig } from "./cli/init.js";
import { nextStepsReport } from "./cli/next-steps.js";
import { preflight } from "./cli/preflight.js";
import { setupReport } from "./cli/setup.js";
import { backgroundServiceDisabledMessage, serviceCommand } from "./cli/service.js";
import { localSmoke } from "./cli/smoke.js";
import { stateHealthReport } from "./cli/state-health.js";
import { readinessReport } from "./cli/readiness.js";
import { liveLaunchReadinessOptions, providerProofLaunchReadinessOptions, readinessOptionsFromFlags } from "./cli/readiness-options.js";
import { releaseEvidenceReportResult } from "./cli/release-evidence.js";
import { verify } from "./cli/verify.js";
import { voiceLoopReport } from "./cli/voice.js";
import { loadConfig } from "./config.js";
import { AssistantRuntime } from "./core/assistant.js";
import { mcpClientConfigReport } from "./core/mcp-client-config.js";
import { providerGuideReport } from "./providers/guide.js";
import { runTelegramBridge } from "./connectors/telegram.js";
import { runWhatsappBridge } from "./connectors/whatsapp.js";
import { runDiscordBridge } from "./connectors/discord.js";
import { runImessageBridge } from "./connectors/imessage.js";
import { runLineBridge } from "./connectors/line.js";
import { runKakaotalkBridge } from "./connectors/kakaotalk.js";
import { runMatrixBridge } from "./connectors/matrix.js";
...
async function main() {
    ...
    switch (parsed.command) {
        ...
        case "launch-check": {
            ...
            if (flagBool(parsed.flags, "strict") && !result.ok)
                process.exitCode = 1;
            return;
        }
        case "launch-status":
        ...
        case "run-jobs":
        case "work":
            if (await runJobsNeedsGate(config, parsed.positionals) && !await foregroundGate("run-jobs", config, parsed.flags))
                return;
        ...
        case "remove-action":
            ...
            return;
        case "scheduler":
            if (config.scheduler.enabled && !await foregroundGate("scheduler", config, parsed.flags))
                return;
        ...
        case "telegram":
            ...
            if (!await foregroundGate("telegram", config, parsed.flags))
                return;
        ...
        case "discord":
            ...
            if (!await foregroundGate("discord", config, parsed.flags))
                return;
        ...
        case "slack":
            ...
            if (!await foregroundGate("slack", config, parsed.flags))
                return;
        ...
        case "matrix":
            ...
            if (!await foregroundGate("matrix", config, parsed.flags))
                return;
        ...
        case "signal":
            ...
            if (!await foregroundGate("signal", config, parsed.flags))
                return;
            await runSignalBridge(config.connectors.signal, assistant, access);
            return;
        case "imessage":
            if (!await foregroundGate("imessage", config, parsed.flags))
                return;
        ...
        case "whatsapp":
            ...
            if (!await foregroundGate("whatsapp", config, parsed.flags))
                return;
        ...
        case "line":
            ...
            if (!await foregroundGate("line", config, parsed.flags))
                return;
        ...
        case "kakaotalk":
            ...
            if (!await foregroundGate("kakaotalk", config, parsed.flags))
                return;
...
async function askOnce(assistant, prompt, sessionId, providerId, options = {}) {
...
async function chatLoop(assistant, sessionId, providerId, options = {}) {
    const rl = createInterface({ input, output });
    if (providerId)
        await assistant.handle(`/provider ${providerId}`, sessionId, { source: "cli" });
...
async function runWebDashboardCommand(config, sessionId, flags) {
...
function configWithWebDashboardFlags(config, flags) {
    const requested = flagBool(flags, "webDashboard") ||
    ...
    if (!requested && host === config.webDashboard.host && port === config.webDashboard.port && allowRemote === config.webDashboard.allowRemote)
        return config;
    if (!port || port > 65_535) {
...
async function firstRunSetupReport() {
    return [
...
async function runJobsNeedsGate(config, positionals) {
    ...
    if (explicitLimit && parsedLimit === undefined)
        return false;
    const pending = await new JobStore(config.jobs.dir).pending(parsedLimit ?? 1);
    return pending.length > 0;
}
function runJobsArgument(positionals, flags) {
    const args = [...positionals];
    const parallel = flagString(flags, "parallel") ?? flagString(flags, "concurrency");
    if (parallel)
        args.push("--parallel", parallel);
    else if (flagBool(flags, "parallel") || flagBool(flags, "concurrency"))
        args.push("--parallel");
    return args.join(" ");
}
function jobWorkerConfig(config, flags) {
    const parallel = flagString(flags, "parallel") ?? flagString(flags, "concurrency");
    if (!parallel && !flagBool(flags, "parallel") && !flagBool(flags, "concurrency"))
        return config;
