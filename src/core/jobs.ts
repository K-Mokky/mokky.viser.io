...
export interface JobProcessor {
  handle(input: string, sessionId: string, options?: AssistantHandleOptions): Promise<string>;
}
...
export class JobStore {
  ...
  async list(status?: QueuedJobStatus): Promise<QueuedJob[]> {
  ...
  async pending(limit = 1, now = new Date()): Promise<QueuedJob[]> {
  ...
  async removeTerminal(id: string): Promise<boolean> {
    const jobs = await this.list();
  ...
  async requeueRunning(): Promise<number> {
    ...
    if (count === 0) return 0;
    await this.writeAll(jobs);
    await this.appendEvent("requeued-running", { id: "batch", status: "pending" } as QueuedJob);
    return count;
  }

  async start(id: string, now = new Date()): Promise<QueuedJob | undefined> {
    const jobs = await this.list();
    const job = jobs.find((item) => item.id === id);
    if (!job || job.status !== "pending") return undefined;
    if (job.nextAttemptAt && new Date(job.nextAttemptAt) > now) return undefined;
    const doneIds = new Set(jobs.filter((item) => item.status === "done").map((item) => item.id));
    if (!dependenciesSatisfied(job, doneIds)) return undefined;
  ...
  async defer(id: string, error: string, now = new Date()): Promise<void> {
    const jobs = await this.list();
    const job = jobs.find((item) => item.id === id);
    if (!job) return;
  ...
  async dependencyContext(ids: string[], maxArtifactChars = 4_000): Promise<string> {
    const jobs = await this.list();
    const byId = new Map(jobs.map((job) => [job.id, job]));
    const sections = ids.map((id) => {
      const job = byId.get(id);
      if (!job) return [`## Dependency job ${id}`, "status: missing"].join("\n");
  ...
  async formatList(status?: QueuedJobStatus): Promise<string> {
    const jobs = await this.list(status);
    if (jobs.length === 0) return status ? `No ${status} jobs.` : "No jobs yet.";
  ...
  private async complete(id: string, patch: Pick<QueuedJob, "status"> & Partial<Pick<QueuedJob, "result" | "error">>): Promise<void> {
    const jobs = await this.list();
    const job = jobs.find((item) => item.id === id);
    if (!job) return;
  ...
  private async writeAll(jobs: QueuedJob[]): Promise<void> {
    await ensurePrivateDir(this.dir);
    await writePrivateFile(this.jobsPath(), `${JSON.stringify(jobs, null, 2)}\n`);
  }

  private async appendEvent(event: string, job: QueuedJob): Promise<void> {
    await ensurePrivateDir(this.dir);
...
export class JobRunner {
  ...
  private options: JobRunOptions;

  constructor(config: JobsConfig, processor: JobProcessor, options: JobRunOptions = {}) {
    this.config = config;
  ...
  async runOnce(limit = this.config.concurrency, options: JobRunOptions = {}): Promise<number> {
  ...
  async loop(): Promise<void> {
    ...
    try {
      ...
      if (requeued > 0) console.log(`Requeued ${requeued} running job(s) from a previous worker session.`);
    } catch (error) {
...
export async function runJobWorkerLoopIteration(
  store: JobStore,
  processor: JobProcessor,
  limit = 1,
  logLine: (message: string) => void = console.log,
    ...
    const outcomes = await mapWithConcurrency(started, concurrency, async (job) => {
      try {
        ...
        if (isProviderFailureOutput(output)) {
          return { job, status: "deferred" as const, output };
        }
        return { job, status: "done" as const, output };
      } catch (error) {
        return { job, status: "failed" as const, output: error instanceof Error ? error.message : String(error) };
      }
    });
...
export function parseJobStatus(value: string): QueuedJobStatus | undefined {
  return ["pending", "running", "done", "failed", "cancelled"].includes(value)
    ? value as QueuedJobStatus
    : undefined;
}

export function providerFailureBackoffMs(attempts: number): number {
  const safeAttempts = Math.max(1, Math.floor(attempts));
  return Math.min(PROVIDER_FAILURE_MAX_BACKOFF_MS, PROVIDER_FAILURE_BASE_BACKOFF_MS * (2 ** (safeAttempts - 1)));
}

function normalizeJobDependencies(dependsOn: string[] | undefined): string[] {
  const output: string[] = [];
  ...
  if (output.length > 20) throw new Error("Job dependency count is too large.");
  return output;
}

function dependenciesSatisfied(job: QueuedJob, doneIds: Set<string>): boolean {
  return (job.dependsOn ?? []).every((id) => doneIds.has(id));
}

async function jobProcessorInput(store: JobStore, job: QueuedJob, maxInputChars?: number): Promise<string> {
  if (!job.dependsOn?.length) return job.prompt;
...
function truncateToCharBudget(value: string, maxChars: number): string {
  const chars = Array.from(value);
  if (chars.length <= maxChars) return value;

  const suffix = `\n[truncated ${chars.length - maxChars} chars to fit job input limit]`;
  const suffixChars = Array.from(suffix);
  if (maxChars <= suffixChars.length) return chars.slice(0, maxChars).join("");
...
function normalizeConcurrency(value: number | undefined, jobCount: number): number {
