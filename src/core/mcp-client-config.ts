...
import type { ViserConfig } from "./types.ts";

export const MCP_CLIENT_TARGETS = ["generic", "claude-desktop", "codex"] as const;

export type McpClientTarget = typeof MCP_CLIENT_TARGETS[number];
...
export interface McpClientConfigResult {
  ...
  clientLabel: string;
  serverName: string;
  clientConfig: McpClientConfigSnippet;
  notes: string[];
}
...
export function mcpClientConfigData(config: ViserConfig, options: McpClientConfigOptions = {}): McpClientConfigResult {
  ...
  const server: McpServerConfigEntry = {
    command: process.execPath,
    args: [INDEX_ENTRYPOINT, "mcp-server"],
    cwd: config.assistant.workdir
  };

  if (config.configPath) server.env = { VISER_CONFIG: config.configPath };
...
export function mcpClientConfigReport(config: ViserConfig, options: McpClientConfigOptions = {}): string {
  try {
...
export function mcpClientConfigUsage(): string {
  return [
...
function normalizeMcpClientTarget(target?: string): McpClientTarget | undefined {
  ...
  return (MCP_CLIENT_TARGETS as readonly string[]).includes(normalized) ? normalized as McpClientTarget : undefined;
}
