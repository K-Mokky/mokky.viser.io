...
import type { PersonalizationSetting, PersonalizationState } from "./types.ts";

export const PERSONALIZATION_STATE_VERSION = 1;

export const PERSONALIZATION_KEY_ALIASES: Record<string, string> = {
  "tone": "ai.tone",
  "ai-tone": "ai.tone",
  "ai.tone": "ai.tone",
  "말투": "ai.tone",
...
export class PersonalizationStore {
  ...
  private promptLimit: number;

  constructor(options: { dir: string; maxValueChars: number; promptLimit: number }) {
    this.dir = options.dir;
    this.maxValueChars = options.maxValueChars;
    this.promptLimit = options.promptLimit;
  }

  async list(limit = 50): Promise<PersonalizationSetting[]> {
  ...
  async get(key: string): Promise<PersonalizationSetting | undefined> {
    const canonicalKey = canonicalPersonalizationKey(key);
    const state = await this.readState();
    return state.settings.find((setting) => setting.key === canonicalKey);
  }

  async set(key: string, value: string, source: string): Promise<PersonalizationSetting> {
    const canonicalKey = canonicalPersonalizationKey(key);
    ...
    const setting: PersonalizationSetting = existing
      ? {
          ...existing,
  ...
  async unset(key: string): Promise<boolean> {
    ...
    if (nextSettings.length === state.settings.length) return false;
  ...
  async handleCommand(argument: string, source: string): Promise<string> {
    const parsed = parsePersonalizationCommand(argument);
    if (parsed.kind === "help") return personalizationHelpText();
    if (parsed.kind === "list") return await this.formatList();
    if (parsed.kind === "clear") {
      if (!parsed.force) return "Usage: /persona clear --force";
  ...
  private async readState(): Promise<PersonalizationState> {
    const raw = await readPrivateFileIfExists(this.filePath(), { dirs: [this.dir] });
    if (raw === undefined) return emptyPersonalizationState();
    return normalizePersonalizationState(JSON.parse(raw));
  }

  private async writeState(state: PersonalizationState): Promise<void> {
...
export type ParsedPersonalizationCommand =
  | { kind: "help" }
  | { kind: "list" }
...
export function parsePersonalizationCommand(argument: string): ParsedPersonalizationCommand {
  const trimmed = argument.trim();
  if (!trimmed) return { kind: "list" };
  if (/^(?:help|-h|--help)$/iu.test(trimmed)) return { kind: "help" };

  const [first = "", ...rest] = trimmed.split(/\s+/u);
  const firstLower = first.toLowerCase();
  if (["list", "show", "ls"].includes(firstLower)) return { kind: "list" };
  ...
  if (["unset", "delete", "remove", "rm"].includes(firstLower)) {
    ...
    if (!key) throw new Error("Usage: /persona unset <key>");
    return { kind: "unset", key };
  }
  if (["set", "write", "save"].includes(firstLower)) {
    const [key = "", ...valueParts] = rest;
    const value = valueParts.join(" ").trim();
    if (!key || !value) throw new Error("Usage: /persona set <key> <value>");
  ...
  if (aliasKey) {
    ...
    if (!value) throw new Error(`Usage: /persona ${first} <value>`);
    return { kind: "set", key: aliasKey, value };
  }

  const value = rest.join(" ").trim();
  if (!value) throw new Error("Usage: /persona [set] <key> <value>");
  return { kind: "set", key: first, value };
}

export function canonicalPersonalizationKey(key: string): string {
  const trimmed = key.trim();
  const aliased = PERSONALIZATION_KEY_ALIASES[trimmed.toLowerCase()];
  return aliased ?? trimmed;
}

export function validatePersonalizationSetting(key: string, value: string, maxValueChars: number): void {
  if (!key) throw new Error("Personalization key is required.");
  ...
  if (!value) throw new Error("Personalization value is required.");
...
export function personalizationSensitivityReasons(value: string): string[] {
  const reasons: string[] = [];
  for (const detector of SENSITIVE_VALUE_PATTERNS) {
    if (detector.pattern.test(value)) reasons.push(detector.id);
  }
  return [...new Set(reasons)];
}

export function normalizePersonalizationState(value: unknown): PersonalizationState {
  if (!isRecord(value)) throw new Error("personalization state must be a JSON object");
  const rawSettings = Array.isArray(value.settings) ? value.settings : [];
  const byKey = new Map<string, PersonalizationSetting>();
...
function personalizationHelpText(): string {
  return [
