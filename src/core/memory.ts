...
import type { MemoryCompactionResult, MemoryEntry, MemoryProfile } from "./types.ts";
...
export class MemoryStore {
  ...
  async add(text: string, options: { tags?: string[]; source: string }): Promise<MemoryEntry> {
    ...
    if (!entry.text) throw new Error("Cannot remember an empty value.");
  ...
  async search(query: string, limit = 10): Promise<MemoryEntry[]> {
    ...
    if (terms.tokens.length === 0 && terms.tags.length === 0) return entries.slice(-limit);
  ...
  async compact(options: { maxEntries?: number } = {}): Promise<MemoryCompactionResult> {
    const entries = await this.readAll();
  ...
  async profile(options: { tagLimit?: number; itemLimitPerTag?: number; untaggedLimit?: number } = {}): Promise<MemoryProfile> {
    const entries = await this.readAll();
  ...
  async formatProfileForPrompt(options: { tagLimit?: number; itemLimitPerTag?: number; untaggedLimit?: number } = {}): Promise<string> {
    const profile = await this.profile(options);
    if (profile.totalCount === 0) return "(none)";

    const lines: string[] = [`total_memories: ${profile.totalCount}`];
    for (const group of profile.groups) {
      lines.push(`- #${group.tag} (${group.count})`);
      for (const entry of group.entries) lines.push(`  - [${entry.id}] ${entry.text}`);
    }

    if (profile.untagged.length > 0) {
      lines.push("- untagged recent");
      for (const entry of profile.untagged) lines.push(`  - [${entry.id}] ${entry.text}`);
    }

    return lines.join("\n");
  }

  async formatForPrompt(query: string, limit: number): Promise<string> {
...
export function parseMemoryInput(input: string): { text: string; tags: string[] } {
...
function parseSearchTerms(value: string): SearchTerms {
...
function scoreEntry(entry: MemoryEntry, terms: SearchTerms): number {
  const text = normalizeText(entry.text);
  const tagSet = new Set(entry.tags.map((tag) => tag.toLowerCase()));
  const source = entry.source.toLowerCase();
  let score = 0;

  if (terms.phrase.length >= 3 && text.includes(terms.phrase)) score += 5;

  for (const tag of terms.tags) {
    if (tagSet.has(tag)) score += 6;
    else if ([...tagSet].some((entryTag) => entryTag.includes(tag))) score += 3;
  }

  for (const token of terms.tokens) {
    if (tagSet.has(token)) score += 4;
  ...
  if (vectorScore >= 0.08) score += vectorScore * 8;

  return score;
}

type LexicalVector = Map<string, number>;
...
function buildLexicalVector(value: string): LexicalVector {
  const vector: LexicalVector = new Map();

  for (const token of tokenize(value)) {
    if (ENGLISH_STOPWORDS.has(token)) continue;

    const pieces = token.split(/[_-]+/u).filter((piece) => piece.length >= 2);
    for (const piece of [token, ...pieces]) addTokenFeatures(vector, piece);
  }

  return vector;
}

function addTokenFeatures(vector: LexicalVector, token: string): void {
  addVectorWeight(vector, `tok:${token}`, 1);

  const stem = stemEnglishToken(token);
  if (stem !== token && stem.length >= 2) addVectorWeight(vector, `stem:${stem}`, 0.8);

  for (const ngram of charNgrams(token, 3)) addVectorWeight(vector, `tri:${ngram}`, 0.35);
}

function stemEnglishToken(token: string): string {
  if (!/^[a-z][a-z0-9-]*$/u.test(token) || token.length < 5) return token;
...
function charNgrams(value: string, size: number): string[] {
  const chars = [...value];
  if (chars.length < size) return chars.length > 0 ? [value] : [];
...
function addVectorWeight(vector: LexicalVector, feature: string, weight: number): void {
  vector.set(feature, (vector.get(feature) ?? 0) + weight);
}

function cosineSimilarity(left: LexicalVector, right: LexicalVector): number {
...
function dedupeByText(entries: MemoryEntry[]): MemoryEntry[] {
