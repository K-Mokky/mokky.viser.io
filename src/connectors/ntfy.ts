...
import type { NtfyConnectorConfig } from "../core/types.ts";
...
export async function sendNtfyMessage(
  config: NtfyConnectorConfig,
  topicAlias: string,
  text: string,
  options: NtfyFetchOptions = {}
): Promise<void> {
...
export interface NtfyTopicTarget {
  alias: string;
  baseUrl: string;
  topic: string;
}

export function resolveNtfyTopicTarget(config: NtfyConnectorConfig, topicAlias: string): NtfyTopicTarget {
...
export function hasNtfyTopic(config: NtfyConnectorConfig): boolean {
  return Boolean(config.topic || Object.keys(config.topics).length > 0);
}

export function normalizeNtfyBaseUrl(value: string | undefined): string {
  ...
  if (!url.hostname) throw new Error("ntfy base URL must include a hostname.");
  if (url.username || url.password) throw new Error("ntfy base URL credentials are not allowed; use NTFY_TOKEN for protected topics.");
  url.search = "";
  url.hash = "";
...
export function normalizeNtfyToken(value: string | undefined): string {
  const token = normalizeOptionalNtfyToken(value);
  if (!token) throw new Error("ntfy token is required for protected topics.");
  return token;
}

export function normalizeOptionalNtfyToken(value: string | undefined): string | undefined {
...
export function normalizeNtfyTopicAlias(value: string | undefined): string {
  const trimmed = value?.trim();
...
export function parseNtfyTopicTarget(value: string): string {
  const raw = value.trim();
  if (!raw) throw new Error("ntfy topic target is required.");
  ...
  if (/^\./u.test(raw) || raw.includes("..")) throw new Error("ntfy topic target must not contain hidden or parent path segments.");
  return raw;
}

export function parseNtfyTopicMap(value: string | undefined): Record<string, string> {
  const raw = value?.trim();
  if (!raw) return {};
  try {
  ...
  for (const part of raw.split(/[,\n;]/u)) {
    ...
    if (alias && target) output[alias] = parseNtfyTopicTarget(target);
  }
  return output;
}

export function redactNtfyDetail(
  detail: string,
  config: NtfyConnectorConfig,
  alias?: string,
  topic?: string,
...
function ntfyPublishUrl(baseUrl: string, topic: string): string {
...
function isLocalNtfyHost(hostname: string): boolean {
