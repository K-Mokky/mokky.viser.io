...
export async function pollImessageMessages(
  config: ImessageConnectorConfig,
  assistant: AssistantRuntime,
  state: ImessagePollState = {},
  access?: AccessStore,
  rateLimiter?: ConnectorRateLimiter,
  options: ImessageRunOptions = {}
): Promise<number> {
  ...
  for (const message of messages) {
    ...
    if (options.processMessages === false) continue;
    await handleImessageMessage(config, assistant, message, access, rateLimiter, options);
  }
  return messages.length;
}

export async function handleImessageMessage(
  config: ImessageConnectorConfig,
  assistant: AssistantRuntime,
  ...
  if (!handleId || !message.text) return;

  const staticAllowlist = [...config.allowedHandleIds, ...config.defaultHandleIds].map((value) => normalizeImessageHandle(value)).filter(Boolean) as string[];
  if (config.allowedHandleIds.length > 0 && !staticAllowlist.includes(handleId)) return;
...
export async function sendImessageMessage(
  ...
  if (!recipient) throw new Error("iMessage handle id is required.");
  const runner = options.runner ?? runCommand;
...
export function parseImessageRows(output: string): ImessageMessage[] {
  ...
  for (const row of parsed) {
    ...
    const record = row as ImessageSqliteRow;
    const rowid = Number(record.rowid);
    const handleId = normalizeImessageHandle(record.handle_id);
    const text = typeof record.text === "string" ? record.text : undefined;
    if (!Number.isSafeInteger(rowid) || rowid <= 0 || !handleId || !text) continue;
    messages.push({ rowid, handleId, text, chatGuid: record.chat_guid });
  }
  return messages;
}

export function normalizeImessageHandle(value: string | undefined): string | undefined {
  const trimmed = value?.trim();
  if (!trimmed || trimmed.length > 200 || /[\u0000-\u001F\u007F]/u.test(trimmed)) return undefined;
  if (/^\+[1-9]\d{4,19}$/u.test(trimmed)) return trimmed;
  if (/^[1-9]\d{4,19}$/u.test(trimmed)) return `+${trimmed}`;
  if (/^[^@\s]+@[^@\s]+\.[^@\s]+$/u.test(trimmed)) return trimmed.toLowerCase();
  return undefined;
}

function imessageQuery(afterRowId: number): string {
...
function imessageSendArgs(recipient: string, text: string): string[] {
  return [
    "-e", "on run argv",
...
function redactImessageDetail(detail: string, config: ImessageConnectorConfig, recipient?: string, text?: string): string {
  let output = detail;
...
function imessageRuntimeEnv(): Record<string, string | undefined> {
  return {
