...
import { normalizeDingTalkWebhookUrl } from "./dingtalk.ts";
import { normalizeFeishuWebhookUrl } from "./feishu.ts";
import { normalizeGenericWebhookInboundToken, normalizeGenericWebhookUrl } from "./generic-webhook.ts";
import { githubHeaders, normalizeGitHubToken, parseGitHubIssueTarget, redactGitHubDetail } from "./github.ts";
import { normalizeTodoistToken, parseTodoistProjectTarget, redactTodoistDetail, todoistHeaders } from "./todoist.ts";
import { normalizeNotionToken, notionHeaders, parseNotionPageTarget, redactNotionDetail } from "./notion.ts";
import { normalizeObsidianNotePath, normalizeObsidianVaultDir, redactObsidianDetail } from "./obsidian.ts";
import { normalizeGoogleChatWebhookUrl } from "./google-chat.ts";
import { normalizeHomeAssistantAccessToken, normalizeHomeAssistantBaseUrl, normalizeHomeAssistantServiceSpec, redactHomeAssistantDetail } from "./home-assistant.ts";
import { normalizeIrcChannel, normalizeIrcHost, normalizeIrcNick, normalizeIrcPassword, normalizeIrcPort, redactIrcDetail } from "./irc.ts";
import { normalizeMattermostWebhookUrl } from "./mattermost.ts";
import { normalizeNextcloudTalkAppPassword, normalizeNextcloudTalkBaseUrl, normalizeNextcloudTalkRoomToken, normalizeNextcloudTalkUsername, redactNextcloudTalkDetail } from "./nextcloud-talk.ts";
import { normalizeNtfyBaseUrl, normalizeOptionalNtfyToken, parseNtfyTopicTarget, redactNtfyDetail } from "./ntfy.ts";
import { mastodonApiUrl, normalizeMastodonAccessToken, normalizeMastodonBaseUrl, normalizeMastodonVisibility, redactMastodonDetail } from "./mastodon.ts";
import { normalizeRocketChatWebhookUrl } from "./rocket-chat.ts";
import { normalizeSynologyChatWebhookUrl } from "./synology-chat.ts";
import { normalizeTeamsWebhookUrl } from "./teams.ts";
import { normalizeTwitchAccessToken, normalizeTwitchChannel, normalizeTwitchUsername, redactTwitchDetail } from "./twitch.ts";
import { normalizeWeComWebhookUrl } from "./wecom.ts";
import { normalizeZaloAccessToken, normalizeZaloUserId, redactZaloDetail } from "./zalo.ts";
import { normalizeZulipApiKey, normalizeZulipBotEmail, normalizeZulipSiteUrl, redactZulipDetail, zulipAuthorizationHeader } from "./zulip.ts";
...
export async function validateSlackToken(
  ...
  if (!token) return { ok: false, label: "slack", detail: "missing token" };
  ...
  try {
    ...
    const body = (await response.json().catch(() => ({}))) as { ok?: boolean; user?: string; error?: string };
    if (!response.ok || !body.ok) {
...
export async function validateMatrixToken(
  ...
  if (!homeserverUrl) return { ok: false, label: "matrix", detail: "missing homeserver URL" };
  if (!token) return { ok: false, label: "matrix", detail: "missing token" };
  ...
  try {
    ...
    const body = (await response.json().catch(() => ({}))) as { user_id?: string; errcode?: string; error?: string };
    if (!response.ok) {
...
export async function validateWhatsappToken(
  ...
  if (!graphApiVersion) return { ok: false, label: "whatsapp", detail: "missing Graph API version" };
  if (!phoneNumberId) return { ok: false, label: "whatsapp", detail: "missing phone number ID" };
  if (!token) return { ok: false, label: "whatsapp", detail: "missing token" };
  ...
  try {
    ...
    const body = (await response.json().catch(() => ({}))) as { id?: string; display_phone_number?: string; error?: { message?: string }; message?: string };
    if (!response.ok) {
...
export async function validateLineToken(
  token: string | undefined,
  fetchImpl: FetchLike = fetch,
  options: TokenValidationOptions = {}
): Promise<TokenValidationResult> {
  if (!token) return { ok: false, label: "line", detail: "missing channel access token" };
  ...
  try {
    ...
    const body = (await response.json().catch(() => ({}))) as { displayName?: string; userId?: string; message?: string };
    if (!response.ok) {
...
export function validateGoogleChatWebhook(webhookUrl: string | undefined): TokenValidationResult {
  if (!webhookUrl) return { ok: false, label: "google-chat", detail: "missing webhook URL" };
  try {
...
export function validateGenericWebhook(webhookUrl: string | undefined): TokenValidationResult {
  if (!webhookUrl) return { ok: false, label: "webhook", detail: "missing webhook URL" };
  try {
...
export function validateGenericInboundWebhookToken(token: string | undefined): TokenValidationResult {
  if (!token) return { ok: false, label: "webhook-inbound", detail: "missing inbound webhook token" };
  try {
...
export async function validateHomeAssistantApi(
  ...
  if (!baseUrl) return { ok: false, label: "home-assistant", detail: "missing base URL" };
  if (!accessToken) return { ok: false, label: "home-assistant", detail: "missing access token" };
  if (!service) return { ok: false, label: "home-assistant", detail: "missing service alias target" };
  try {
...
export function validateTeamsWebhook(webhookUrl: string | undefined): TokenValidationResult {
  if (!webhookUrl) return { ok: false, label: "teams", detail: "missing webhook URL" };
  try {
...
export function validateMattermostWebhook(webhookUrl: string | undefined): TokenValidationResult {
  if (!webhookUrl) return { ok: false, label: "mattermost", detail: "missing webhook URL" };
  try {
...
export function validateSynologyChatWebhook(webhookUrl: string | undefined): TokenValidationResult {
  if (!webhookUrl) return { ok: false, label: "synology-chat", detail: "missing webhook URL" };
  try {
...
export function validateRocketChatWebhook(webhookUrl: string | undefined): TokenValidationResult {
  if (!webhookUrl) return { ok: false, label: "rocket-chat", detail: "missing webhook URL" };
  try {
...
export function validateFeishuWebhook(webhookUrl: string | undefined): TokenValidationResult {
  if (!webhookUrl) return { ok: false, label: "feishu", detail: "missing webhook URL" };
  try {
...
export function validateDingTalkWebhook(webhookUrl: string | undefined): TokenValidationResult {
  if (!webhookUrl) return { ok: false, label: "dingtalk", detail: "missing webhook URL" };
  try {
...
export function validateWeComWebhook(webhookUrl: string | undefined): TokenValidationResult {
  if (!webhookUrl) return { ok: false, label: "wecom", detail: "missing webhook URL" };
  try {
...
export function validateIrcConfig(host: string | undefined, port: number, nick: string | undefined, channel: string | undefined, password?: string): TokenValidationResult {
  if (!host) return { ok: false, label: "irc", detail: "missing host" };
  if (!nick) return { ok: false, label: "irc", detail: "missing nick" };
  if (!channel) return { ok: false, label: "irc", detail: "missing channel" };
  const redactionConfig = {
    enabled: true,
    hostEnv: "IRC_HOST",
    host,
...
export function validateTwitchConfig(accessToken: string | undefined, botUsername: string | undefined, channel: string | undefined): TokenValidationResult {
  if (!accessToken) return { ok: false, label: "twitch", detail: "missing OAuth token" };
  if (!botUsername) return { ok: false, label: "twitch", detail: "missing bot username" };
  if (!channel) return { ok: false, label: "twitch", detail: "missing channel" };
  const redactionConfig = {
    enabled: true,
...
export function validateNextcloudTalkConfig(
  baseUrl: string | undefined,
  username: string | undefined,
  appPassword: string | undefined,
  roomToken: string | undefined
): TokenValidationResult {
  if (!baseUrl) return { ok: false, label: "nextcloud-talk", detail: "missing base URL" };
  if (!username) return { ok: false, label: "nextcloud-talk", detail: "missing username" };
  if (!appPassword) return { ok: false, label: "nextcloud-talk", detail: "missing app password" };
  if (!roomToken) return { ok: false, label: "nextcloud-talk", detail: "missing room token" };
  const redactionConfig = {
    enabled: true,
...
export function validateZaloCredentials(accessToken: string | undefined, recipient: string | undefined): TokenValidationResult {
  if (!accessToken) return { ok: false, label: "zalo", detail: "missing access token" };
  if (!recipient) return { ok: false, label: "zalo", detail: "missing recipient" };
  const redactionConfig = {
    enabled: true,
...
export async function validateWebexToken(
  token: string | undefined,
  fetchImpl: FetchLike = fetch,
  options: TokenValidationOptions = {}
): Promise<TokenValidationResult> {
  if (!token) return { ok: false, label: "webex", detail: "missing token" };
  ...
  try {
    ...
    const body = (await response.json().catch(() => ({}))) as { displayName?: string; message?: string; errors?: Array<{ description?: string }> };
    if (!response.ok) {
...
export async function validateZulipToken(
  ...
  if (!siteUrl) return { ok: false, label: "zulip", detail: "missing site URL" };
  if (!botEmail) return { ok: false, label: "zulip", detail: "missing bot email" };
  if (!apiKey) return { ok: false, label: "zulip", detail: "missing API key" };
  ...
  try {
    ...
    const body = (await response.json().catch(() => ({}))) as { full_name?: string; email?: string; msg?: string; result?: string };
...
export async function validateGitHubToken(
  ...
  if (!token) return { ok: false, label: "github", detail: "missing token" };
  ...
  try {
    ...
    const body = (await response.json().catch(() => ({}))) as { login?: string; message?: string };
    if (!response.ok) {
...
export function validateGitHubTarget(
  token: string | undefined,
  target: string | undefined
): TokenValidationResult {
  if (!token) return { ok: false, label: "github", detail: "missing token" };
  if (!target) return { ok: false, label: "github", detail: "missing issue target" };
  const redactionConfig = {
    enabled: true,
    tokenEnv: "GITHUB_TOKEN",
    token,
...
export async function validateTodoistToken(
  ...
  if (!token) return { ok: false, label: "todoist", detail: "missing token" };
...
export function validateTodoistTarget(
  token: string | undefined,
  ...
  try {
    ...
    if (project) parseTodoistProjectTarget(project);
...
export function validateNtfyTarget(
  baseUrl: string | undefined,
  token: string | undefined,
  topic: string | undefined
): TokenValidationResult {
  if (!baseUrl) return { ok: false, label: "ntfy", detail: "missing base URL" };
  if (!topic) return { ok: false, label: "ntfy", detail: "missing topic target" };
  const redactionConfig = {
    enabled: true,
...
export async function validateMastodonToken(
  baseUrl: string | undefined,
  accessToken: string | undefined,
  ...
  if (!baseUrl) return { ok: false, label: "mastodon", detail: "missing base URL" };
  if (!accessToken) return { ok: false, label: "mastodon", detail: "missing access token" };
  ...
  try {
    ...
    const body = (await response.json().catch(() => ({}))) as { acct?: string; username?: string; error?: string };
    if (!response.ok) {
...
export async function validateNotionToken(
  ...
  if (!token) return { ok: false, label: "notion", detail: "missing token" };
  ...
  try {
    ...
    const body = (await response.json().catch(() => ({}))) as { name?: string; type?: string; message?: string };
    if (!response.ok) {
...
export function validateNotionTarget(
  token: string | undefined,
  page: string | undefined
): TokenValidationResult {
  if (!token) return { ok: false, label: "notion", detail: "missing token" };
  if (!page) return { ok: false, label: "notion", detail: "missing page target" };
  const redactionConfig = {
    enabled: true,
    tokenEnv: "NOTION_TOKEN",
    token,
...
export function validateObsidianTarget(
  vaultDir: string | undefined,
  note: string | undefined
): TokenValidationResult {
  if (!vaultDir) return { ok: false, label: "obsidian", detail: "missing vault dir" };
  if (!note) return { ok: false, label: "obsidian", detail: "missing note target" };
  const redactionConfig = {
    enabled: true,
...
export function validateSignalCli(
  command: string | undefined,
  account: string | undefined
): TokenValidationResult {
  if (!account) return { ok: false, label: "signal", detail: "missing account" };
  if (!command) return { ok: false, label: "signal", detail: "missing command" };
  if (!commandExists(command)) return { ok: false, label: "signal", detail: `command '${command}' missing` };
  return { ok: true, label: "signal", detail: "local signal-cli configured" };
}

export function validateImessageLocal(
  sqliteCommand: string | undefined,
  osascriptCommand: string | undefined,
  chatDbPath: string | undefined
): TokenValidationResult {
  if (!chatDbPath) return { ok: false, label: "imessage", detail: "missing chat database path" };
  if (!sqliteCommand) return { ok: false, label: "imessage", detail: "missing sqlite command" };
  if (!osascriptCommand) return { ok: false, label: "imessage", detail: "missing osascript command" };
  if (!commandExists(sqliteCommand)) return { ok: false, label: "imessage", detail: `command '${sqliteCommand}' missing` };
  if (!commandExists(osascriptCommand)) return { ok: false, label: "imessage", detail: `command '${osascriptCommand}' missing` };
  try {
...
function redactMany(detail: string, values: Array<string | undefined>): string {
  let output = detail;
