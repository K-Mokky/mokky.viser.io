...
import type { WebexConnectorConfig } from "../core/types.ts";

const WEBEX_MESSAGES_URL = "https://webexapis.com/v1/messages";
const WEBEX_CHUNK_SIZE = 3900;

export interface WebexFetchOptions {
  fetchImpl?: FetchLike;
}

export async function sendWebexMessage(
  config: WebexConnectorConfig,
  roomId: string,
  text: string,
  options: WebexFetchOptions = {}
): Promise<void> {
  if (!config.accessToken) throw new Error(`Webex access token is missing. Set ${config.accessTokenEnv}.`);
...
export function normalizeWebexRoomId(value: string): string {
  const trimmed = value.trim();
  if (!trimmed) throw new Error("Webex room ID is required.");
...
export function redactWebexDetail(detail: string, config: WebexConnectorConfig, roomId?: string, text?: string): string {
  let output = detail;
