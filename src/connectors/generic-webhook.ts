...
import { connectorInputTooLong } from "./input-policy.ts";
import { chunkText } from "../utils/text.ts";
import type { AssistantRuntime } from "../core/assistant.ts";
import type { GenericWebhookConnectorConfig } from "../core/types.ts";
...
export interface GenericWebhookFetchOptions {
  fetchImpl?: FetchLike;
}

export interface GenericWebhookInboundAttachment {
  name?: string;
  type?: string;
  url?: string;
  text?: string;
...
export interface GenericWebhookInboundResult {
  sessionId: string;
  sourceId: string;
...
export async function sendGenericWebhookMessage(
  config: GenericWebhookConnectorConfig,
  webhookId: string,
  text: string,
  options: GenericWebhookFetchOptions = {}
): Promise<void> {
...
export function resolveGenericWebhookUrl(config: GenericWebhookConnectorConfig, webhookId: string): string {
  const id = normalizeGenericWebhookId(webhookId);
  if (!id) throw new Error("Generic webhook id must be a short alias such as default or ops.");
...
export function normalizeGenericWebhookId(value: string | undefined): string | undefined {
  const trimmed = value?.trim();
  if (!trimmed || trimmed.length > 80) return undefined;
  if (/^[A-Za-z0-9._-]+$/u.test(trimmed)) return trimmed.toLowerCase();
  return undefined;
}

export function parseGenericWebhookUrlMap(value: string | undefined): Record<string, string> {
...
export function normalizeGenericWebhookUrl(value: string): string {
  const trimmed = value.trim();
  let url: URL;
  try {
    url = new URL(trimmed);
...
export function hasGenericWebhook(config: GenericWebhookConnectorConfig): boolean {
  return Boolean(config.webhookUrl || Object.keys(config.webhookUrls).length > 0);
}

export function normalizeGenericWebhookInboundToken(value: string | undefined): string {
...
export function normalizeGenericWebhookInboundPath(value: string | undefined): string {
...
export function hasGenericInboundWebhook(config: GenericWebhookConnectorConfig): boolean {
  return Boolean(config.inboundEnabled && config.inboundToken);
}

export function genericWebhookInboundBodyLimitBytes(config: GenericWebhookConnectorConfig): number {
...
export async function handleGenericWebhookInbound(
  config: GenericWebhookConnectorConfig,
  payload: unknown,
  assistant: AssistantRuntime
): Promise<GenericWebhookInboundResult> {
...
function normalizeGenericWebhookInboundPayload(
  payload: unknown,
  maxInputChars: number
): { text: string; sourceId: string; providerId?: string; attachments: GenericWebhookInboundAttachment[] } {
  const record = payload && typeof payload === "object" && !Array.isArray(payload) ? payload as Record<string, unknown> : undefined;
  if (!record) throw new Error("Generic inbound webhook payload must be a JSON object.");
...
function normalizeInboundText(value: unknown, maxInputChars: number, required: boolean): string {
  const text = typeof value === "string" ? value.replace(/\r\n?/gu, "\n").trim() : "";
  if (!text) {
    if (required) throw new Error("Generic inbound webhook text is required unless attachments are supplied.");
    return "Review the inbound webhook attachment context.";
  }
  if (connectorInputTooLong(text, maxInputChars)) throw new Error(`Generic inbound webhook text must be ${maxInputChars} characters or fewer.`);
  if (/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/u.test(text)) throw new Error("Generic inbound webhook text contains control characters.");
  return text;
}

function normalizeInboundProviderId(value: unknown): string | undefined {
...
function normalizeInboundAttachments(value: unknown): GenericWebhookInboundAttachment[] {
  if (value === undefined) return [];
  if (!Array.isArray(value)) throw new Error("Generic inbound webhook attachments must be an array when present.");
...
function normalizeInboundAttachment(value: unknown, index: number): GenericWebhookInboundAttachment {
  const record = value && typeof value === "object" && !Array.isArray(value) ? value as Record<string, unknown> : undefined;
  if (!record) throw new Error(`Generic inbound webhook attachment ${index + 1} must be a JSON object.`);

  const attachment: GenericWebhookInboundAttachment = {};
...
function normalizeAttachmentMetadata(value: unknown, field: "name" | "type", index: number): string | undefined {
...
function normalizeAttachmentUrl(value: unknown, index: number): string | undefined {
  if (value === undefined || value === null) return undefined;
...
function normalizeAttachmentText(value: unknown, index: number): string | undefined {
  if (value === undefined || value === null) return undefined;
  const text = typeof value === "string" ? value.replace(/\r\n?/gu, "\n").trim() : "";
  if (!text) return undefined;
...
function normalizeAttachmentSize(value: unknown, index: number): number | undefined {
  if (value === undefined || value === null) return undefined;
...
function composeGenericWebhookInboundPrompt(text: string, attachments: GenericWebhookInboundAttachment[]): string {
  ...
  attachments.forEach((attachment, index) => {
    ...
    if (attachment.text) lines.push(`   text: ${attachment.text}`);
  });
  return lines.join("\n");
}

function formatAttachmentSummary(attachment: GenericWebhookInboundAttachment): string {
  const parts = [
...
export function redactGenericWebhookDetail(detail: string, config: GenericWebhookConnectorConfig, webhookId?: string, text?: string): string {
  let output = detail;
