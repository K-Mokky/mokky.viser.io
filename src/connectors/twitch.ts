...
export interface TwitchRunOptions {
  runner?: (input: TwitchSendInput) => Promise<void>;
}

export async function sendTwitchMessage(
  config: TwitchConnectorConfig,
  channelId: string,
  text: string,
  options: TwitchRunOptions = {}
): Promise<void> {
...
export function resolveTwitchChannel(config: TwitchConnectorConfig, channelId: string): string {
...
export function hasTwitchChannel(config: TwitchConnectorConfig): boolean {
  return Boolean(config.channel || Object.keys(config.channels).length > 0);
}

export function parseTwitchChannelMap(raw: string | undefined): Record<string, string> {
...
export function normalizeTwitchChannelAlias(value: string | undefined): string {
  const id = normalizeWebhookId(value);
  if (!id) throw new Error("Twitch channel alias must be a short alias such as default or ops.");
  return id.toLowerCase();
}

export function normalizeTwitchUsername(value: string | undefined): string {
  const username = value?.trim().replace(/^@/u, "").toLowerCase() ?? "";
  if (!username) throw new Error("Twitch bot username is required.");
...
export function normalizeTwitchAccessToken(value: string | undefined): string {
  const raw = value?.trim() ?? "";
  if (!raw) throw new Error("Twitch OAuth token is required.");
...
export function normalizeTwitchChannel(value: string | undefined): string {
  const raw = value?.trim().replace(/^#/u, "").toLowerCase() ?? "";
  if (!raw) throw new Error("Twitch channel is required.");
...
export function redactTwitchDetail(detail: string, config: TwitchConnectorConfig, alias?: string, channel?: string, text?: string): string {
  let output = detail;
...
async function runTwitchSend(input: TwitchSendInput): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    ...
    socket.on("end", () => {
      if (!settled) fail(new Error("server closed before delivery"));
    });
  });
}

function normalizeTwitchMessageBody(raw: string): string {
  if (/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/u.test(raw)) throw new Error("Twitch message contains control characters.");
  const text = raw.replace(/\r\n?/gu, "\n").split("\n").map((line) => line.trim()).filter(Boolean).join(" / ").trim();
  if (!text) throw new Error("Twitch message is required.");
  if (text.length > 20_000) throw new Error("Twitch message is too long.");
  return text;
}
