...
import type { ObsidianConnectorConfig } from "../core/types.ts";
...
export interface ObsidianAppendOptions {
  now?: Date;
}

export async function appendObsidianNoteMessage(
  config: ObsidianConnectorConfig,
  noteId: string,
  text: string,
  options: ObsidianAppendOptions = {}
): Promise<void> {
...
export function resolveObsidianNoteTarget(config: ObsidianConnectorConfig, noteId: string): ObsidianNoteTarget {
...
export function hasObsidianNoteTarget(config: ObsidianConnectorConfig): boolean {
  return Boolean(config.vaultDir && (config.note || Object.keys(config.notes).length > 0));
}

export function normalizeObsidianVaultDir(value: string | undefined): string {
  const trimmed = value?.trim();
  if (!trimmed) throw new Error("Obsidian vault directory is required.");
  if (/[\u0000-\u001f\u007f]/u.test(trimmed)) throw new Error("Obsidian vault directory must not contain control characters.");
  return resolve(trimmed);
}

export function normalizeObsidianTargetAlias(value: string | undefined): string {
  const trimmed = value?.trim();
...
export function normalizeObsidianNotePath(value: string): string {
  const trimmed = value.trim();
  if (!trimmed) throw new Error("Obsidian note path is required.");
  if (trimmed.length > 512) throw new Error("Obsidian note path must be 512 characters or fewer.");
  if (isAbsolute(trimmed) || /^[A-Za-z]:[\\/]/u.test(trimmed)) throw new Error("Obsidian note path must be relative to the vault.");
  ...
  if (!/\.md$/iu.test(normalized)) throw new Error("Obsidian note path must end with .md.");
  return parts.join("/");
}

export function parseObsidianNoteMap(value: string | undefined): Record<string, string> {
  ...
  for (const part of raw.split(/[,\n;]/u)) {
    ...
    if (alias && target) output[alias] = normalizeObsidianNotePath(target);
  }
  return output;
}

export function formatObsidianNoteTarget(target: ObsidianNoteTarget): string {
  return target.notePath;
}

export function redactObsidianDetail(
  detail: string,
  config: ObsidianConnectorConfig,
  alias?: string,
...
function formatObsidianAppend(text: string, now: Date): string {
  return `\n\n## Viser — ${now.toISOString()}\n\n${text}\n`;
}

function normalizeObsidianMessage(text: string, maxMessageChars: number): string {
  const normalized = text.replace(/\r\n?/gu, "\n").trim();
  if (!normalized) throw new Error("Obsidian note message body is required.");
  if (normalized.length > maxMessageChars) throw new Error(`Obsidian note message body must be ${maxMessageChars} characters or fewer.`);
  return normalized;
}

async function assertSafeObsidianVaultRoot(vaultDir: string): Promise<void> {
  const info = await lstat(vaultDir);
  if (info.isSymbolicLink()) throw new Error("Obsidian vault directory must not be a symlink.");
  if (!info.isDirectory()) throw new Error("Obsidian vault path must be a directory.");
}

function assertPathInsideObsidianVault(vaultDir: string, absolutePath: string): void {
