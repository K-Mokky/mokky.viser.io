...
import type { TodoistConnectorConfig } from "../core/types.ts";
...
export interface TodoistProjectTarget {
  alias: string;
  projectId?: string;
}

export interface TodoistFetchOptions {
  fetchImpl?: FetchLike;
  uuidFactory?: () => string;
}

interface TodoistTaskParts {
  content: string;
  description?: string;
}

export async function sendTodoistTask(
  config: TodoistConnectorConfig,
  projectAlias: string,
  text: string,
  options: TodoistFetchOptions = {}
): Promise<void> {
...
export function resolveTodoistProjectTarget(config: TodoistConnectorConfig, projectAlias: string): TodoistProjectTarget {
  const alias = normalizeTodoistTargetAlias(projectAlias);
  if (alias === "inbox") return { alias };
...
export function hasTodoistCredentials(config: TodoistConnectorConfig): boolean {
  return Boolean(config.token);
}

export function hasTodoistProjectTarget(config: TodoistConnectorConfig): boolean {
  return Boolean(config.project || Object.keys(config.projects).length > 0);
}

export function normalizeTodoistToken(value: string | undefined): string {
  const trimmed = value?.trim();
  if (!trimmed) throw new Error("Todoist API token is required.");
...
export function normalizeTodoistTargetAlias(value: string | undefined): string {
  const trimmed = value?.trim();
...
export function parseTodoistProjectTarget(value: string): string {
  const raw = value.trim();
  if (!raw) throw new Error("Todoist project target is required.");
  if (/^tmp-/iu.test(raw)) throw new Error("Todoist project target must be a synced server project ID, not a tmp-* placeholder.");
...
export function parseTodoistProjectMap(value: string | undefined): Record<string, string> {
  const raw = value?.trim();
  if (!raw) return {};
  try {
  ...
  for (const part of raw.split(/[,\n;]/u)) {
    ...
    if (alias && target) output[alias] = parseTodoistProjectTarget(target);
  }
  return output;
}

export function todoistHeaders(token: string): Record<string, string> {
  return {
...
export function redactTodoistDetail(
  detail: string,
  config: TodoistConnectorConfig,
  alias?: string,
  target?: TodoistProjectTarget,
  text?: string
): string {
  let output = detail;
...
function normalizeTodoistTaskMessage(text: string): TodoistTaskParts {
  const normalized = text.replace(/\r\n?/gu, "\n").trim();
  if (!normalized) throw new Error("Todoist task content is required.");
  const [firstLine = "", ...rest] = normalized.split("\n");
  const content = firstLine.trim();
  if (!content) throw new Error("Todoist task content is required.");
...
function parseTodoistSyncResponse(bodyText: string): { sync_status?: Record<string, unknown> } {
  try {
    const parsed = JSON.parse(bodyText) as unknown;
    if (typeof parsed === "object" && parsed !== null) return parsed as { sync_status?: Record<string, unknown> };
  } catch {
