...
export async function readinessItems(config: ViserConfig, options: ReadinessOptions = {}): Promise<ReadinessItem[]> {
  ...
  if (config.personalization.enabled) items.push(await writableDirItem("personalization", config.personalization.dir));
  ...
  if (config.webDashboard.enabled) items.push(await writableDirItem("web-dashboard", config.webDashboard.canvasDir));
  if (config.webDashboard.enabled) items.push(webDashboardAuthReadinessItem(config));
  ...
  addConnectorStaticChecks(
    items,
    "matrix",
    config.connectors.matrix.enabled,
    Boolean(config.connectors.matrix.accessToken && config.connectors.matrix.homeserverUrl),
    `${config.connectors.matrix.homeserverUrlEnv}/${config.connectors.matrix.accessTokenEnv}`
  );
  addConnectorStaticChecks(
    items,
    "signal",
    config.connectors.signal.enabled,
    Boolean(config.connectors.signal.account && commandExists(config.connectors.signal.command)),
    `${config.connectors.signal.accountEnv}/${config.connectors.signal.commandEnv}`
  );
  addConnectorStaticChecks(
    items,
    "imessage",
    config.connectors.imessage.enabled,
    Boolean(config.connectors.imessage.enabled && commandExists(config.connectors.imessage.sqliteCommand) && commandExists(config.connectors.imessage.osascriptCommand)),
    `${config.connectors.imessage.sqliteCommandEnv}/${config.connectors.imessage.osascriptCommandEnv}/${config.connectors.imessage.chatDbPathEnv}`
  );
  addConnectorStaticChecks(
    items,
    "whatsapp",
    config.connectors.whatsapp.enabled,
    Boolean(config.connectors.whatsapp.accessToken && config.connectors.whatsapp.phoneNumberId && config.connectors.whatsapp.verifyToken),
  ...
  addConnectorStaticChecks(
    items,
    "line",
    config.connectors.line.enabled,
    Boolean(config.connectors.line.channelAccessToken && config.connectors.line.channelSecret),
    `${config.connectors.line.channelAccessTokenEnv}/${config.connectors.line.channelSecretEnv}`
  );
  addConnectorStaticChecks(
    items,
    "kakaotalk",
    config.connectors.kakaotalk.enabled,
    Boolean(config.connectors.kakaotalk.requestToken),
    config.connectors.kakaotalk.requestTokenEnv
  );
  addConnectorStaticChecks(
    items,
    "google-chat",
    config.connectors.googleChat.enabled,
    hasWebhook(config.connectors.googleChat),
    `${config.connectors.googleChat.webhookUrlEnv}/${config.connectors.googleChat.webhookUrlsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "webhook",
    config.connectors.webhook.enabled,
    hasWebhook(config.connectors.webhook),
    `${config.connectors.webhook.webhookUrlEnv}/${config.connectors.webhook.webhookUrlsEnv}`
  );
  ...
  addConnectorStaticChecks(
    items,
    "home-assistant",
    config.connectors.homeAssistant.enabled,
    Boolean(config.connectors.homeAssistant.baseUrl && config.connectors.homeAssistant.accessToken && hasHomeAssistantService(config.connectors.homeAssistant)),
    `${config.connectors.homeAssistant.baseUrlEnv}/${config.connectors.homeAssistant.accessTokenEnv}/${config.connectors.homeAssistant.serviceEnv}/${config.connectors.homeAssistant.servicesEnv}`
  );
  addConnectorStaticChecks(
    items,
    "teams",
    config.connectors.teams.enabled,
    hasWebhook(config.connectors.teams),
    `${config.connectors.teams.webhookUrlEnv}/${config.connectors.teams.webhookUrlsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "mattermost",
    config.connectors.mattermost.enabled,
    hasWebhook(config.connectors.mattermost),
    `${config.connectors.mattermost.webhookUrlEnv}/${config.connectors.mattermost.webhookUrlsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "synology-chat",
    config.connectors.synologyChat.enabled,
    hasWebhook(config.connectors.synologyChat),
    `${config.connectors.synologyChat.webhookUrlEnv}/${config.connectors.synologyChat.webhookUrlsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "rocket-chat",
    config.connectors.rocketChat.enabled,
    hasWebhook(config.connectors.rocketChat),
    `${config.connectors.rocketChat.webhookUrlEnv}/${config.connectors.rocketChat.webhookUrlsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "feishu",
    config.connectors.feishu.enabled,
    hasWebhook(config.connectors.feishu),
    `${config.connectors.feishu.webhookUrlEnv}/${config.connectors.feishu.webhookUrlsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "dingtalk",
    config.connectors.dingtalk.enabled,
    hasWebhook(config.connectors.dingtalk),
    `${config.connectors.dingtalk.webhookUrlEnv}/${config.connectors.dingtalk.webhookUrlsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "wecom",
    config.connectors.wecom.enabled,
    hasWebhook(config.connectors.wecom),
    `${config.connectors.wecom.webhookUrlEnv}/${config.connectors.wecom.webhookUrlsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "zalo",
    config.connectors.zalo.enabled,
    Boolean(config.connectors.zalo.accessToken && hasZaloRecipient(config.connectors.zalo)),
    `${config.connectors.zalo.accessTokenEnv}/${config.connectors.zalo.recipientEnv}/${config.connectors.zalo.recipientsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "irc",
    config.connectors.irc.enabled,
    Boolean(config.connectors.irc.host && config.connectors.irc.nick && hasIrcChannel(config.connectors.irc)),
    `${config.connectors.irc.hostEnv}/${config.connectors.irc.portEnv}/${config.connectors.irc.tlsEnv}/${config.connectors.irc.nickEnv}/${config.connectors.irc.passwordEnv}/${config.connectors.irc.channelEnv}/${config.connectors.irc.channelsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "twitch",
    config.connectors.twitch.enabled,
    Boolean(config.connectors.twitch.accessToken && config.connectors.twitch.botUsername && hasTwitchChannel(config.connectors.twitch)),
    `${config.connectors.twitch.accessTokenEnv}/${config.connectors.twitch.botUsernameEnv}/${config.connectors.twitch.channelEnv}/${config.connectors.twitch.channelsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "ntfy",
    config.connectors.ntfy.enabled,
    hasNtfyTopic(config.connectors.ntfy),
    `${config.connectors.ntfy.baseUrlEnv}/${config.connectors.ntfy.tokenEnv}/${config.connectors.ntfy.topicEnv}/${config.connectors.ntfy.topicsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "mastodon",
    config.connectors.mastodon.enabled,
    Boolean(config.connectors.mastodon.baseUrl && config.connectors.mastodon.accessToken),
    `${config.connectors.mastodon.baseUrlEnv}/${config.connectors.mastodon.accessTokenEnv}/${config.connectors.mastodon.visibilityEnv}/${config.connectors.mastodon.targetsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "nextcloud-talk",
    config.connectors.nextcloudTalk.enabled,
    Boolean(config.connectors.nextcloudTalk.baseUrl && config.connectors.nextcloudTalk.username && config.connectors.nextcloudTalk.appPassword && hasNextcloudTalkRoom(config.connectors.nextcloudTalk)),
    `${config.connectors.nextcloudTalk.baseUrlEnv}/${config.connectors.nextcloudTalk.usernameEnv}/${config.connectors.nextcloudTalk.appPasswordEnv}/${config.connectors.nextcloudTalk.roomTokenEnv}/${config.connectors.nextcloudTalk.roomsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "webex",
    config.connectors.webex.enabled,
    Boolean(config.connectors.webex.accessToken),
    config.connectors.webex.accessTokenEnv
  );
  addConnectorStaticChecks(
    items,
    "zulip",
    config.connectors.zulip.enabled,
    hasZulipCredentials(config.connectors.zulip) && hasZulipTarget(config.connectors.zulip),
    `${config.connectors.zulip.siteUrlEnv}/${config.connectors.zulip.botEmailEnv}/${config.connectors.zulip.apiKeyEnv}/${config.connectors.zulip.targetEnv}/${config.connectors.zulip.targetsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "email",
    config.connectors.email.enabled,
    Boolean(config.connectors.email.from && hasEmailRecipient(config.connectors.email) && commandExists(config.connectors.email.sendmailCommand)),
    `${config.connectors.email.sendmailCommandEnv}/${config.connectors.email.fromEnv}/${config.connectors.email.recipientEnv}/${config.connectors.email.recipientsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "github",
    config.connectors.github.enabled,
    Boolean(config.connectors.github.token && hasGitHubTarget(config.connectors.github)),
    `${config.connectors.github.tokenEnv}/${config.connectors.github.targetEnv}/${config.connectors.github.targetsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "todoist",
    config.connectors.todoist.enabled,
    Boolean(config.connectors.todoist.token),
    `${config.connectors.todoist.tokenEnv}/${config.connectors.todoist.projectEnv}/${config.connectors.todoist.projectsEnv}`
  );
  addConnectorStaticChecks(
    items,
    "notion",
    config.connectors.notion.enabled,
    Boolean(config.connectors.notion.token && hasNotionPage(config.connectors.notion)),
    `${config.connectors.notion.tokenEnv}/${config.connectors.notion.pageEnv}/${config.connectors.notion.pagesEnv}`
  );
  addConnectorStaticChecks(
    items,
    "obsidian",
    config.connectors.obsidian.enabled,
    Boolean(config.connectors.obsidian.vaultDir && hasObsidianNote(config.connectors.obsidian)),
    `${config.connectors.obsidian.vaultDirEnv}/${config.connectors.obsidian.noteEnv}/${config.connectors.obsidian.notesEnv}`
  );
  ...
  if (options.live) {
    ...
    items.push(liveConnectorItem(
      ...
      slack.ok,
      slack.detail,
      Boolean(config.connectors.slack.enabled),
      Boolean(config.connectors.slack.botToken),
    ...
    items.push(liveConnectorItem(
      ...
      matrix.ok,
      matrix.detail,
      Boolean(config.connectors.matrix.enabled),
      Boolean(config.connectors.matrix.accessToken && config.connectors.matrix.homeserverUrl),
      `${config.connectors.matrix.homeserverUrlEnv}/${config.connectors.matrix.accessTokenEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      signal.ok,
      signal.detail,
      Boolean(config.connectors.signal.enabled),
      Boolean(config.connectors.signal.account),
      `${config.connectors.signal.accountEnv}/${config.connectors.signal.commandEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      imessage.ok,
      imessage.detail,
      Boolean(config.connectors.imessage.enabled),
      Boolean(config.connectors.imessage.enabled),
      `${config.connectors.imessage.sqliteCommandEnv}/${config.connectors.imessage.osascriptCommandEnv}/${config.connectors.imessage.chatDbPathEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      whatsapp.ok,
      whatsapp.detail,
      Boolean(config.connectors.whatsapp.enabled),
      Boolean(config.connectors.whatsapp.accessToken && config.connectors.whatsapp.phoneNumberId),
      `${config.connectors.whatsapp.accessTokenEnv}/${config.connectors.whatsapp.phoneNumberIdEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      line.ok,
      line.detail,
      Boolean(config.connectors.line.enabled),
      Boolean(config.connectors.line.channelAccessToken && config.connectors.line.channelSecret),
      `${config.connectors.line.channelAccessTokenEnv}/${config.connectors.line.channelSecretEnv}`
    ));
    items.push(liveConnectorItem(
      "kakaotalk",
      Boolean(config.connectors.kakaotalk.requestToken),
      config.connectors.kakaotalk.requestToken ? "shared Skill token configured" : "missing KakaoTalk Skill shared token",
      Boolean(config.connectors.kakaotalk.enabled),
      Boolean(config.connectors.kakaotalk.requestToken),
      config.connectors.kakaotalk.requestTokenEnv
    ));
    ...
    items.push(liveConnectorItem(
      ...
      googleChat.ok,
      googleChat.detail,
      Boolean(config.connectors.googleChat.enabled),
      hasWebhook(config.connectors.googleChat),
      `${config.connectors.googleChat.webhookUrlEnv}/${config.connectors.googleChat.webhookUrlsEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      "webhook",
      genericWebhook.ok,
      genericWebhook.detail,
      Boolean(config.connectors.webhook.enabled),
      hasWebhook(config.connectors.webhook),
      `${config.connectors.webhook.webhookUrlEnv}/${config.connectors.webhook.webhookUrlsEnv}`
    ));
    const homeAssistant = await validateHomeAssistantApi(
      config.connectors.homeAssistant.baseUrl,
      config.connectors.homeAssistant.accessToken,
      firstHomeAssistantService(config.connectors.homeAssistant)
    );
    items.push(liveConnectorItem(
      "home-assistant",
      homeAssistant.ok,
      homeAssistant.detail,
      Boolean(config.connectors.homeAssistant.enabled),
      Boolean(config.connectors.homeAssistant.baseUrl && config.connectors.homeAssistant.accessToken && hasHomeAssistantService(config.connectors.homeAssistant)),
      `${config.connectors.homeAssistant.baseUrlEnv}/${config.connectors.homeAssistant.accessTokenEnv}/${config.connectors.homeAssistant.serviceEnv}/${config.connectors.homeAssistant.servicesEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      teams.ok,
      teams.detail,
      Boolean(config.connectors.teams.enabled),
      hasWebhook(config.connectors.teams),
      `${config.connectors.teams.webhookUrlEnv}/${config.connectors.teams.webhookUrlsEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      mattermost.ok,
      mattermost.detail,
      Boolean(config.connectors.mattermost.enabled),
      hasWebhook(config.connectors.mattermost),
      `${config.connectors.mattermost.webhookUrlEnv}/${config.connectors.mattermost.webhookUrlsEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      synologyChat.ok,
      synologyChat.detail,
      Boolean(config.connectors.synologyChat.enabled),
      hasWebhook(config.connectors.synologyChat),
      `${config.connectors.synologyChat.webhookUrlEnv}/${config.connectors.synologyChat.webhookUrlsEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      rocketChat.ok,
      rocketChat.detail,
      Boolean(config.connectors.rocketChat.enabled),
      hasWebhook(config.connectors.rocketChat),
      `${config.connectors.rocketChat.webhookUrlEnv}/${config.connectors.rocketChat.webhookUrlsEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      feishu.ok,
      feishu.detail,
      Boolean(config.connectors.feishu.enabled),
      hasWebhook(config.connectors.feishu),
      `${config.connectors.feishu.webhookUrlEnv}/${config.connectors.feishu.webhookUrlsEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      dingTalk.ok,
      dingTalk.detail,
      Boolean(config.connectors.dingtalk.enabled),
      hasWebhook(config.connectors.dingtalk),
      `${config.connectors.dingtalk.webhookUrlEnv}/${config.connectors.dingtalk.webhookUrlsEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      weCom.ok,
      weCom.detail,
      Boolean(config.connectors.wecom.enabled),
      hasWebhook(config.connectors.wecom),
      `${config.connectors.wecom.webhookUrlEnv}/${config.connectors.wecom.webhookUrlsEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      zalo.ok,
      zalo.detail,
      Boolean(config.connectors.zalo.enabled),
      Boolean(config.connectors.zalo.accessToken && hasZaloRecipient(config.connectors.zalo)),
      `${config.connectors.zalo.accessTokenEnv}/${config.connectors.zalo.recipientEnv}/${config.connectors.zalo.recipientsEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      irc.ok,
      irc.detail,
      Boolean(config.connectors.irc.enabled),
      Boolean(config.connectors.irc.host && config.connectors.irc.nick && hasIrcChannel(config.connectors.irc)),
      `${config.connectors.irc.hostEnv}/${config.connectors.irc.portEnv}/${config.connectors.irc.tlsEnv}/${config.connectors.irc.nickEnv}/${config.connectors.irc.passwordEnv}/${config.connectors.irc.channelEnv}/${config.connectors.irc.channelsEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      twitch.ok,
      twitch.detail,
      Boolean(config.connectors.twitch.enabled),
      Boolean(config.connectors.twitch.accessToken && config.connectors.twitch.botUsername && hasTwitchChannel(config.connectors.twitch)),
      `${config.connectors.twitch.accessTokenEnv}/${config.connectors.twitch.botUsernameEnv}/${config.connectors.twitch.channelEnv}/${config.connectors.twitch.channelsEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      ntfy.ok,
      ntfy.detail,
      Boolean(config.connectors.ntfy.enabled),
      hasNtfyTopic(config.connectors.ntfy),
      `${config.connectors.ntfy.baseUrlEnv}/${config.connectors.ntfy.tokenEnv}/${config.connectors.ntfy.topicEnv}/${config.connectors.ntfy.topicsEnv}`
    ));
    const mastodon = await validateMastodonToken(
      config.connectors.mastodon.baseUrl,
      config.connectors.mastodon.accessToken,
      config.connectors.mastodon.visibility
    );
    items.push(liveConnectorItem(
      "mastodon",
      mastodon.ok,
      mastodon.detail,
      Boolean(config.connectors.mastodon.enabled),
      Boolean(config.connectors.mastodon.baseUrl && config.connectors.mastodon.accessToken),
      `${config.connectors.mastodon.baseUrlEnv}/${config.connectors.mastodon.accessTokenEnv}/${config.connectors.mastodon.visibilityEnv}/${config.connectors.mastodon.targetsEnv}`
    ));
    const nextcloudTalk = validateNextcloudTalkConfig(
      config.connectors.nextcloudTalk.baseUrl,
      config.connectors.nextcloudTalk.username,
      config.connectors.nextcloudTalk.appPassword,
      firstNextcloudTalkRoom(config.connectors.nextcloudTalk)
    );
    items.push(liveConnectorItem(
      "nextcloud-talk",
      nextcloudTalk.ok,
      nextcloudTalk.detail,
      Boolean(config.connectors.nextcloudTalk.enabled),
      Boolean(config.connectors.nextcloudTalk.baseUrl && config.connectors.nextcloudTalk.username && config.connectors.nextcloudTalk.appPassword && hasNextcloudTalkRoom(config.connectors.nextcloudTalk)),
      `${config.connectors.nextcloudTalk.baseUrlEnv}/${config.connectors.nextcloudTalk.usernameEnv}/${config.connectors.nextcloudTalk.appPasswordEnv}/${config.connectors.nextcloudTalk.roomTokenEnv}/${config.connectors.nextcloudTalk.roomsEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      webex.ok,
      webex.detail,
      Boolean(config.connectors.webex.enabled),
      Boolean(config.connectors.webex.accessToken),
      config.connectors.webex.accessTokenEnv
    ));
    ...
    items.push(liveConnectorItem(
      ...
      zulip.ok,
      zulip.detail,
      Boolean(config.connectors.zulip.enabled),
      hasZulipCredentials(config.connectors.zulip),
      `${config.connectors.zulip.siteUrlEnv}/${config.connectors.zulip.botEmailEnv}/${config.connectors.zulip.apiKeyEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      "email",
      emailReady,
      emailReady ? "local sendmail command and envelope are configured" : "missing local sendmail command, from address, or recipient alias",
      Boolean(config.connectors.email.enabled),
      Boolean(config.connectors.email.from && hasEmailRecipient(config.connectors.email)),
      `${config.connectors.email.sendmailCommandEnv}/${config.connectors.email.fromEnv}/${config.connectors.email.recipientEnv}/${config.connectors.email.recipientsEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      github.ok,
      github.detail,
      Boolean(config.connectors.github.enabled),
      Boolean(config.connectors.github.token || hasGitHubTarget(config.connectors.github)),
      `${config.connectors.github.tokenEnv}/${config.connectors.github.targetEnv}/${config.connectors.github.targetsEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      todoist.ok,
      todoist.detail,
      Boolean(config.connectors.todoist.enabled),
      Boolean(config.connectors.todoist.token),
      `${config.connectors.todoist.tokenEnv}/${config.connectors.todoist.projectEnv}/${config.connectors.todoist.projectsEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      notion.ok,
      notion.detail,
      Boolean(config.connectors.notion.enabled),
      Boolean(config.connectors.notion.token || hasNotionPage(config.connectors.notion)),
      `${config.connectors.notion.tokenEnv}/${config.connectors.notion.pageEnv}/${config.connectors.notion.pagesEnv}`
    ));
    ...
    items.push(liveConnectorItem(
      ...
      obsidian.ok,
      obsidian.detail,
      Boolean(config.connectors.obsidian.enabled),
      Boolean(config.connectors.obsidian.vaultDir || hasObsidianNote(config.connectors.obsidian)),
      `${config.connectors.obsidian.vaultDirEnv}/${config.connectors.obsidian.noteEnv}/${config.connectors.obsidian.notesEnv}`
    ));
  }
...
function firstWebhookUrl(config: { webhookUrl?: string; webhookUrls: Record<string, string> }): string | undefined {
  return config.webhookUrl ?? Object.values(config.webhookUrls)[0];
}

function hasHomeAssistantService(config: { service?: string; services: Record<string, string> }): boolean {
  return Boolean(config.service || Object.keys(config.services).length > 0);
}

function firstHomeAssistantService(config: { service?: string; services: Record<string, string> }): string | undefined {
...
function hasEmailRecipient(config: { recipient?: string; recipients: Record<string, string> }): boolean {
  return Boolean(config.recipient || Object.keys(config.recipients).length > 0);
}

function hasGitHubTarget(config: { target?: string; targets: Record<string, string> }): boolean {
  return Boolean(config.target || Object.keys(config.targets).length > 0);
}

function firstGitHubTarget(config: { target?: string; targets: Record<string, string> }): string | undefined {
  return config.target ?? Object.values(config.targets)[0];
}

function firstTodoistProject(config: { project?: string; projects: Record<string, string> }): string | undefined {
  return config.project ?? Object.values(config.projects)[0];
}

function hasNotionPage(config: { page?: string; pages: Record<string, string> }): boolean {
  return Boolean(config.page || Object.keys(config.pages).length > 0);
}

function firstNotionPage(config: { page?: string; pages: Record<string, string> }): string | undefined {
  return config.page ?? Object.values(config.pages)[0];
}

function hasObsidianNote(config: { note?: string; notes: Record<string, string> }): boolean {
  return Boolean(config.note || Object.keys(config.notes).length > 0);
}

function firstObsidianNote(config: { note?: string; notes: Record<string, string> }): string | undefined {
  return config.note ?? Object.values(config.notes)[0];
}

function hasIrcChannel(config: { channel?: string; channels: Record<string, string> }): boolean {
  return Boolean(config.channel || Object.keys(config.channels).length > 0);
}

function firstIrcChannel(config: { channel?: string; channels: Record<string, string> }): string | undefined {
  return config.channel ?? Object.values(config.channels)[0];
}

function hasTwitchChannel(config: { channel?: string; channels: Record<string, string> }): boolean {
  return Boolean(config.channel || Object.keys(config.channels).length > 0);
}

function firstTwitchChannel(config: { channel?: string; channels: Record<string, string> }): string | undefined {
  return config.channel ?? Object.values(config.channels)[0];
}

function hasNtfyTopic(config: { topic?: string; topics: Record<string, string> }): boolean {
  return Boolean(config.topic || Object.keys(config.topics).length > 0);
}

function firstNtfyTopic(config: { topic?: string; topics: Record<string, string> }): string | undefined {
  return config.topic ?? Object.values(config.topics)[0];
}

function hasNextcloudTalkRoom(config: { roomToken?: string; rooms: Record<string, string> }): boolean {
  return Boolean(config.roomToken || Object.keys(config.rooms).length > 0);
}

function firstNextcloudTalkRoom(config: { roomToken?: string; rooms: Record<string, string> }): string | undefined {
  return config.roomToken ?? Object.values(config.rooms)[0];
}

function hasZaloRecipient(config: { recipient?: string; recipients: Record<string, string> }): boolean {
  return Boolean(config.recipient || Object.keys(config.recipients).length > 0);
}

function firstZaloRecipient(config: { recipient?: string; recipients: Record<string, string> }): string | undefined {
...
function webDashboardAuthReadinessItem(config: ViserConfig): ReadinessItem {
