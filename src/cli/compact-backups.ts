...
import { assertNoSymlinkComponentsUnderRoot, PRIVATE_FILE_MODE, removePrivateFileIfExists } from "../utils/files.ts";
import type { ViserConfig } from "../core/types.ts";

export interface CompactBackupOptions {
  delete?: boolean;
  force?: boolean;
  fixPermissions?: boolean;
}

interface CompactBackupArtifact {
  area: "storage" | "memory" | "sessions";
  path: string;
  relativePath: string;
  bytes: number;
  mode?: number;
...
type DirectoryInspection = "ok" | "missing" | CompactBackupArtifact;

export async function compactBackupReport(config: ViserConfig, options: CompactBackupOptions = {}): Promise<string> {
...
async function listCompactBackups(
  config: ViserConfig,
  options: { fixPermissions: boolean }
): Promise<CompactBackupArtifact[]> {
  ...
  if (storageRootWarning === "missing") return [];
  if (storageRootWarning !== "ok") return [storageRootWarning];
...
async function collectArea(
  dir: string,
  area: CompactBackupArtifact["area"],
  storageDir: string,
  root: string,
  ...
  if (directoryWarning === "missing") return [];
  if (directoryWarning !== "ok") return [directoryWarning];

  const artifacts: CompactBackupArtifact[] = [];
  ...
  for (const name of names.filter(isBackup).sort()) {
    ...
    if (isBroad && options.fixPermissions) await chmodCompactBackupNoFollow(path);
    artifacts.push({
      area,
      path,
      relativePath,
...
async function chmodCompactBackupNoFollow(path: string): Promise<void> {
  let handle: Awaited<ReturnType<typeof open>> | undefined;
  try {
    handle = await open(path, constants.O_RDONLY | constants.O_NOFOLLOW);
    const info = await handle.stat();
    if (!info.isFile()) throw new Error(`compact backup artifact is not a regular file: ${path}`);
...
function formatArtifacts(artifacts: CompactBackupArtifact[]): string[] {
...
function isMemoryCompactBackup(name: string): boolean {
  return /\.bak\.jsonl$/iu.test(name);
}

function isSessionCompactBackup(name: string): boolean {
  return /\.bak(?:\.jsonl)?$/iu.test(name);
}
