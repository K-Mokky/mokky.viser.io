...
function messengerSteps(config: ViserConfig, readiness: ReadinessItem[]): string[] {
  ...
  if (liveChecks.length > 0) {
    ...
    if (slack?.status !== "pass") lines.push(`- Slack: set ${config.connectors.slack.botTokenEnv} and ${config.connectors.slack.appTokenEnv} if you want Slack messaging.`);
    if (matrix?.status !== "pass") lines.push(`- Matrix: set ${config.connectors.matrix.homeserverUrlEnv} and ${config.connectors.matrix.accessTokenEnv} if you want Matrix messaging.`);
    if (signal?.status !== "pass") lines.push(`- Signal: set ${config.connectors.signal.accountEnv} and optionally ${config.connectors.signal.commandEnv} if you want Signal messaging through local signal-cli.`);
    if (imessage?.status !== "pass") lines.push(`- iMessage: keep ${config.connectors.imessage.sqliteCommandEnv}, ${config.connectors.imessage.osascriptCommandEnv}, and ${config.connectors.imessage.chatDbPathEnv} valid if you want local macOS Messages bridging.`);
    if (whatsapp?.status !== "pass") lines.push(`- WhatsApp: set ${config.connectors.whatsapp.accessTokenEnv}, ${config.connectors.whatsapp.phoneNumberIdEnv}, and ${config.connectors.whatsapp.verifyTokenEnv} if you want WhatsApp Cloud API messaging.`);
    if (line?.status !== "pass") lines.push(`- LINE: set ${config.connectors.line.channelAccessTokenEnv} and ${config.connectors.line.channelSecretEnv} if you want LINE Messaging API webhooks and push replies.`);
    if (kakaotalk?.status !== "pass") lines.push(`- KakaoTalk: set ${config.connectors.kakaotalk.requestTokenEnv} if you want Kakao i Open Builder Skill webhooks.`);
    if (googleChat?.status !== "pass") lines.push(`- Google Chat: set ${config.connectors.googleChat.webhookUrlEnv} or ${config.connectors.googleChat.webhookUrlsEnv} if you want outbound Google Chat messages.`);
    if (genericWebhook?.status !== "pass") lines.push(`- Generic webhook: set ${config.connectors.webhook.webhookUrlEnv} or ${config.connectors.webhook.webhookUrlsEnv} if you want outbound custom HTTPS webhook messages.`);
    if (homeAssistant?.status !== "pass") lines.push(`- Home Assistant: set ${config.connectors.homeAssistant.baseUrlEnv}, ${config.connectors.homeAssistant.accessTokenEnv}, and ${config.connectors.homeAssistant.serviceEnv}/${config.connectors.homeAssistant.servicesEnv} if you want approval-gated smart-home service calls.`);
    if (teams?.status !== "pass") lines.push(`- Microsoft Teams: set ${config.connectors.teams.webhookUrlEnv} or ${config.connectors.teams.webhookUrlsEnv} if you want outbound Teams messages.`);
    if (mattermost?.status !== "pass") lines.push(`- Mattermost: set ${config.connectors.mattermost.webhookUrlEnv} or ${config.connectors.mattermost.webhookUrlsEnv} if you want outbound Mattermost messages.`);
    if (synologyChat?.status !== "pass") lines.push(`- Synology Chat: set ${config.connectors.synologyChat.webhookUrlEnv} or ${config.connectors.synologyChat.webhookUrlsEnv} if you want outbound Synology Chat messages.`);
    if (rocketChat?.status !== "pass") lines.push(`- Rocket.Chat: set ${config.connectors.rocketChat.webhookUrlEnv} or ${config.connectors.rocketChat.webhookUrlsEnv} if you want outbound Rocket.Chat messages.`);
    if (feishu?.status !== "pass") lines.push(`- Feishu: set ${config.connectors.feishu.webhookUrlEnv} or ${config.connectors.feishu.webhookUrlsEnv} if you want outbound Feishu/Lark messages.`);
    if (dingTalk?.status !== "pass") lines.push(`- DingTalk: set ${config.connectors.dingtalk.webhookUrlEnv} or ${config.connectors.dingtalk.webhookUrlsEnv} if you want outbound DingTalk robot messages.`);
    if (weCom?.status !== "pass") lines.push(`- WeCom: set ${config.connectors.wecom.webhookUrlEnv} or ${config.connectors.wecom.webhookUrlsEnv} if you want outbound WeCom group robot messages.`);
    if (zalo?.status !== "pass") lines.push(`- Zalo: set ${config.connectors.zalo.accessTokenEnv} and ${config.connectors.zalo.recipientEnv}/${config.connectors.zalo.recipientsEnv} if you want outbound Zalo OA messages.`);
    if (irc?.status !== "pass") lines.push(`- IRC: set ${config.connectors.irc.hostEnv}, ${config.connectors.irc.nickEnv}, and ${config.connectors.irc.channelEnv}/${config.connectors.irc.channelsEnv} if you want outbound IRC channel messages.`);
    if (twitch?.status !== "pass") lines.push(`- Twitch: set ${config.connectors.twitch.accessTokenEnv}, ${config.connectors.twitch.botUsernameEnv}, and ${config.connectors.twitch.channelEnv}/${config.connectors.twitch.channelsEnv} if you want outbound Twitch chat messages.`);
    if (ntfy?.status !== "pass") lines.push(`- ntfy: set ${config.connectors.ntfy.topicEnv}/${config.connectors.ntfy.topicsEnv} and optionally ${config.connectors.ntfy.baseUrlEnv}/${config.connectors.ntfy.tokenEnv} if you want outbound push notifications.`);
    if (mastodon?.status !== "pass") lines.push(`- Mastodon: set ${config.connectors.mastodon.baseUrlEnv}, ${config.connectors.mastodon.accessTokenEnv}, and optionally ${config.connectors.mastodon.visibilityEnv}/${config.connectors.mastodon.targetsEnv} if you want outbound Fediverse status posts.`);
    if (nextcloudTalk?.status !== "pass") lines.push(`- Nextcloud Talk: set ${config.connectors.nextcloudTalk.baseUrlEnv}, ${config.connectors.nextcloudTalk.usernameEnv}, ${config.connectors.nextcloudTalk.appPasswordEnv}, and ${config.connectors.nextcloudTalk.roomTokenEnv}/${config.connectors.nextcloudTalk.roomsEnv} if you want outbound Talk messages.`);
    if (webex?.status !== "pass") lines.push(`- Webex: set ${config.connectors.webex.accessTokenEnv} if you want outbound Webex Messages API messages.`);
    if (zulip?.status !== "pass") lines.push(`- Zulip: set ${config.connectors.zulip.siteUrlEnv}, ${config.connectors.zulip.botEmailEnv}, ${config.connectors.zulip.apiKeyEnv}, and ${config.connectors.zulip.targetEnv}/${config.connectors.zulip.targetsEnv} if you want outbound Zulip messages.`);
    if (email?.status !== "pass") lines.push(`- Email: set ${config.connectors.email.sendmailCommandEnv}, ${config.connectors.email.fromEnv}, and ${config.connectors.email.recipientEnv}/${config.connectors.email.recipientsEnv} if you want outbound local sendmail messages.`);
    if (github?.status !== "pass") lines.push(`- GitHub: set ${config.connectors.github.tokenEnv} and ${config.connectors.github.targetEnv}/${config.connectors.github.targetsEnv} if you want outbound issue/PR comments.`);
    if (todoist?.status !== "pass") lines.push(`- Todoist: set ${config.connectors.todoist.tokenEnv} and optionally ${config.connectors.todoist.projectEnv}/${config.connectors.todoist.projectsEnv} if you want outbound Todoist task creation.`);
    if (notion?.status !== "pass") lines.push(`- Notion: set ${config.connectors.notion.tokenEnv} and ${config.connectors.notion.pageEnv}/${config.connectors.notion.pagesEnv} if you want outbound Notion page appends.`);
    if (obsidian?.status !== "pass") lines.push(`- Obsidian: set ${config.connectors.obsidian.vaultDirEnv} and ${config.connectors.obsidian.noteEnv}/${config.connectors.obsidian.notesEnv} if you want outbound local Markdown note appends.`);
  }
