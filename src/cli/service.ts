...
export interface ServiceControlOptions {
  ...
  /**
   * Deprecated and ignored. Kept only so older internal callers do not regain
   * background-service behavior by wiring a service gate directly.
   */
  preflightRunner?: (config: ViserConfig) => Promise<unknown>;
}

export function backgroundServiceDisabledMessage(command = "service"): string {
  return [
...
async function serviceManagerCommand(config: ViserConfig, action: "status" | "stop", options: ServiceControlOptions = {}): Promise<string> {
  const platform = options.platform ?? process.platform;
  if (platform === "darwin") {
...
async function launchctlCommand(config: ViserConfig, action: "print" | "kill", options: ServiceControlOptions = {}): Promise<string> {
  const platform = options.platform ?? process.platform;
...
function launchctlHints(output: string): string[] {
  ...
  const hints: string[] = [];
  if (normalized.includes("could not find service")) hints.push("next: no background service is expected; run `viser` in a foreground terminal to start Viser.");
