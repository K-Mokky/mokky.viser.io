...
export async function writeExampleConfig(force = false): Promise<string> {
  const target = resolve(process.cwd(), "viser.config.json");
  const existing = await inspectConfigTarget(target);
  if (existing && !force) {
    if (existing === "symlink") throw new Error(`viser.config.json is a symlink; refusing to treat it as an existing safe config: ${target}`);
    if (existing === "non-file") throw new Error(`viser.config.json exists and is not a regular file: ${target}`);
...
async function inspectConfigTarget(target: string): Promise<"file" | "symlink" | "non-file" | undefined> {
  try {
