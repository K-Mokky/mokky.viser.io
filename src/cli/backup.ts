...
import { assertNoSymlinkComponentsUnderRoot, ensurePrivateDir, writePrivateFile } from "../utils/files.ts";
...
export async function createBackupReport(config: ViserConfig, options: BackupOptions = {}): Promise<string> {
...
): Promise<{ path: string; artifact: BackupArtifact; truncatedFiles: string[]; redactedFiles: string[] }> {
...
function resolveOutputPath(config: ViserConfig, outputPath?: string): string {
  if (outputPath) return isAbsolute(outputPath) ? resolve(outputPath) : resolve(process.cwd(), outputPath);
  return join(config.storage.dir, "backups", `viser-backup-${fileSafeTimestamp(nowIso())}.json`);
}

async function assertSafeBackupOutputTarget(path: string, force: boolean): Promise<void> {
  try {
    ...
    if (!force) throw new Error(`Backup already exists: ${path}. Use --force to overwrite.`);
  } catch (error) {
...
async function collectStorageFiles(
  ...
  if (!await isSafeStorageRoot(storageDir, root)) return [];
  ...
  await walk(storageDir, async (path) => {
    ...
    if (isExcludedStoragePath(relativePath)) return;
    if (resolved === resolve(outputPath)) return;
    if (isInside(resolved, outputRoot) && basename(outputRoot) === "backups") return;
...
export async function readBackupFileNoFollow(path: string): Promise<Buffer | undefined> {
...
async function isSafeStorageRoot(storageDir: string, root: string): Promise<boolean> {
  try {
...
function isExcludedStoragePath(path: string): boolean {
...
function isCompactBackupArtifact(top: string | undefined, leaf: string): boolean {
  if (top === "memory") return /\.bak\.jsonl$/iu.test(leaf);
  if (top === "sessions") return /\.bak(?:\.jsonl)?$/iu.test(leaf);
  return false;
}

function isAtomicTempFileName(name: string): boolean {
  return /^\..+\.\d+\.[0-9a-f-]+\.tmp$/iu.test(name);
}

async function walk(root: string, visit: (path: string) => Promise<void>, shouldDescend: (path: string) => boolean): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    if (info.isDirectory()) {
      if (shouldDescend(path)) await walk(path, visit, shouldDescend);
      continue;
    }
    await visit(path);
  }
...
function sanitizeConfig(config: ViserConfig): unknown {
