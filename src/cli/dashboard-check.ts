...
async function collectLiveDashboardItems(
  target: string,
  fetchImpl: FetchLike,
  timeoutMs: number,
  items: DashboardCheckItem[],
...
async function fetchJson(fetchImpl: FetchLike, url: string, timeoutMs: number, authToken?: string): Promise<unknown> {
...
function requestHeaders(accept: string, authToken?: string): Record<string, string> {
