...
import { assertNoSymlinkComponentsUnderRoot } from "./files.ts";
...
export async function loadEnvFile(path = ".env", baseDir = process.cwd(), options: EnvLoadOptions = {}): Promise<EnvLoadResult> {
  const resolved = resolve(baseDir, path);
  const result: EnvLoadResult = { path: resolved, loaded: [], skipped: [], required: options.required || undefined };
  await assertNoSymlinkComponentsUnderRoot(resolved, baseDir);
  const raw = await readEnvFileNoFollow(resolved);
  if (raw === undefined) {
    if (options.required) throw new Error(`Env file does not exist: ${resolved}`);
...
export async function readEnvFileNoFollow(path: string): Promise<string | undefined> {
  let handle: Awaited<ReturnType<typeof open>> | undefined;
  try {
    handle = await open(path, constants.O_RDONLY | constants.O_NOFOLLOW);
    const info = await handle.stat();
    if (!info.isFile()) throw new Error(`Env path is not a regular file: ${path}`);
...
function readQuotedValue(value: string): string {
  const quote = value[0];
